...
public class MutationGenerator {
  ...
  public void generateMutations(
      IPackageFragment pack, AnalysisParams analysisParams, IMutationExperimentMonitor monitor)
    ...
    for (IJavaElement javaElement : pack.getChildren()) {
      if (javaElement instanceof IPackageFragment) {
        ...
        for (MethodDeclaration testingMethod : testingMethods) {
          ...
          AnalysisTestcaseParams tcParams =
              new AnalysisTestcaseParams(
                  pack.getJavaProject().getElementName(),
                  className,
                  methodName,
                  analysisParams,
                  projectFolder);
          try {
            ...
            if (testSourceFolder == null) {
              testSourceFolder =
                  IResourceUtils.getSourceFolderPath(tcParams.getProjectName(), className);
            }
  ...
  private void collectTestcases(AnalysisTestcaseParams tcParams) {
    FileUtils.appendFile(
        "/Users/lylytran/Projects/jfreechart-tcs.txt", tcParams.getTestcaseName() + "\n");
  }
  ...
  public TraceExecutionInfo executeTestcase(AnalysisTestcaseParams params) {
    ...
    String traceExecPath =
        new StringBuilder(outputFolder).append(File.separator).append("fix.exec").toString();
    String precheckPath = traceExecPath.replace("fix.exec", "precheck.info");
    InstrumentationExecutor executor =
        new InstrumentationExecutor(
            testcaseConfig, traceExecPath, includedClassNames, excludedClassNames);
    executor.setTimeout(params.getAnalysisParams().getExecutionTimeout());
    PreCheckInformation precheckInfo =
        executor.runPrecheck(precheckPath, params.getAnalysisParams().getStepLimit());

    if (!precheckInfo.isPassTest() || precheckInfo.isOverLong()) {
      System.out.println(
          params.getTestcaseName() + " is failed to get build trace: " + precheckInfo.toString());
    ...
    return new TraceExecutionInfo(
        precheckInfo, correctTrace, executor.getTraceExecFilePath(), precheckPath);
  }

  public boolean runSingleTestcase(
      TraceExecutionInfo correctTrace,
      AnalysisTestcaseParams params,
      IMutationExperimentMonitor monitor)
      throws JavaModelException {
    ...
    for (SingleMutation mutation : mutations) {
      ...
      if (!checker.accept(
          mutation.getMutationBugId(), MutationType.valueOf(mutation.getMutationType()))) {
        continue;
      }
      try {
        MutationTrace muTrace =
            generateMutationTrace(correctTrace.getTrace().getAppJavaClassPath(), params, mutation);
        ICompilationUnit iunit =
            JavaUtil.findNonCacheICompilationUnitInProject(
  ...
  private void checkRootCause(
      SingleMutation mutation,
      String orgFilePath,
      String mutationFilePath,
      TraceExecutionInfo mutationTraceInfo,
    ...
    AppJavaClassPathWrapper.wrapAppClassPath(
        mutationTraceInfo.getTrace(), correctTraceInfo.getTrace(), params.getBkClassFiles());
    ...
    DiffMatcher diffMatcher =
        new MuDiffMatcher(mutation.getSourceFolder(), orgFilePath, mutationFilePath);
    diffMatcher.matchCode();
    ControlPathBasedTraceMatcher traceMatcher = new ControlPathBasedTraceMatcher();
    PairList pairList =
        traceMatcher.matchTraceNodePair(killingMutatantTrace, correctTrace, diffMatcher);
    ...
    while (trialNum < trialLimit) {
      ...
      Simulator simulator =
          new Simulator(
              params.getAnalysisParams().isUseSliceBreaker(),
              false,
      ...
      rootcauseFinder.checkRootCause(
          simulator.getObservedFault(), killingMutatantTrace, correctTrace, pairList, diffMatcher);
      TraceNode rootCause =
          rootcauseFinder.retrieveRootCause(
              pairList, diffMatcher, killingMutatantTrace, correctTrace);
      foundRootCause = (rootCause != null);
      boolean includedClassChanged = false;
      if (rootCause == null) {
        System.out.println(
            "[Search Lib Class] Cannot find the root cause, I am searching for library classes...");
        ...
        List<String> newIncludedBuggyClassNames =
            RegressionUtil.identifyIncludedClassNames(
                buggySteps, buggyPrecheck, rootcauseFinder.getRegressionNodeList());
        List<String> newIncludedCorrectClassNames =
            RegressionUtil.identifyIncludedClassNames(
                correctSteps, correctPrecheck, rootcauseFinder.getCorrectNodeList());
      ...
      if (!includedClassChanged) {
        ...
        /* !foundRootCause */
        killingMutatantTrace =
            generateMutatedTrace(
                params,
                mutation,
                testCaseConfig,
                buggyPrecheck,
                ...
                excludedClassNames);
        correctTrace =
            generateCorrectTrace(
                params, testCaseConfig, correctPrecheck, includedClassNames, excludedClassNames);
        killingMutatantTrace.setAppJavaClassPath(
            mutationTraceInfo.getTrace().getAppJavaClassPath());
  ...
  public void runSimulator(
    ...
    List<EmpiricalTrial> trials =
        simulator.detectMutatedBug(killingMutatantTrace, correctTrace, diffMatcher, 0);
    ...
    if (!trials.isEmpty()) {
      ...
      for (DeadEndRecord record : t.getDeadEndRecordList()) {
        ...
        try {
          new DeadEndCSVWriter("_mutation", null)
              .export(datas.getAllData(), params.getProjectName(), mutation.getMutationBugId());
        } catch (IOException e) {
          e.printStackTrace();
        }
      }
  ...
  private Trace generateMutatedTrace(
      AnalysisTestcaseParams params,
      SingleMutation mutation,
      AppJavaClassPath testcaseConfig,
      PreCheckInformation buggyPrecheck,
    ...
    InstrumentationExecutor executor =
        new InstrumentationExecutor(
            testcaseConfig, traceDir, "bug", includedClassNames, excludedClassNames);
    executor.setTimeout(params.getAnalysisParams().getExecutionTimeout());
    RunningInfo runningInfo = executor.execute(buggyPrecheck);
    return runningInfo.getTraceList().get(0);
  }

  private Trace generateCorrectTrace(
      AnalysisTestcaseParams params,
      AppJavaClassPath testcaseConfig,
      PreCheckInformation correctPrecheck,
      List<String> includedClassNames,
      List<String> excludedClassNames) {
    String outputFolder = params.getAnalysisOutputFolder();
    ...
    InstrumentationExecutor executor =
        new InstrumentationExecutor(
            testcaseConfig, outputFolder, "fix", includedClassNames, excludedClassNames);
    executor.setTimeout(params.getAnalysisParams().getExecutionTimeout());
  ...
  private List<ClassLocation> findStaticMutationLocation(
      String junitClassName, List<ClassLocation> mutationLocs, AppJavaClassPath testcaseConfig) {
    Map<String, List<microbat.model.ClassLocation>> class2PointMap =
        BreakpointUtils.initBrkpsMap(toMicrobatClassLocation(mutationLocs));
  ...
  private List<microbat.model.ClassLocation> toMicrobatClassLocation(List<ClassLocation> locs) {
    ...
    for (ClassLocation loc : locs) {
      result.add(
          new microbat.model.ClassLocation(
              loc.getClassCanonicalName(), loc.getMethodSign(), loc.getLineNo()));
    }
    return result;
  }

  private List<ClassLocation> convertClassLocation(
      List<microbat.model.ClassLocation> visitedLocations) {
    List<ClassLocation> locs = new ArrayList<>(visitedLocations.size());
    for (microbat.model.ClassLocation loc : visitedLocations) {
      locs.add(
          new ClassLocation(loc.getClassCanonicalName(), loc.getMethodSign(), loc.getLineNumber()));
    }
    return locs;
  }

  private MutationTrace executeTestcaseWithMutation(
      AppJavaClassPath testcaseConfig, AnalysisParams analysisParams, SingleMutation mutation) {
    ...
    try {
      ...
      List<String> excludedClassNames = AnalysisScopePreference.getExcludedLibList();
      String traceExecPath =
          new StringBuilder(traceDir).append(File.separator).append("bug.exec").toString();
      ...
      PreCheckInformation precheck =
          executor.runPrecheck(precheckPath, analysisParams.getStepLimit());

      muTrace.setTimeOut(precheck.isTimeout());
      muTrace.setKill(!precheck.isPassTest() && !precheck.isTimeout());

      String testMethod =
          testcaseConfig.getOptionalTestClass() + "#" + testcaseConfig.getOptionalTestMethod();

      if (muTrace.isKill()) {
        System.out.println(
            "KILLED: Now generating trace for "
                + testMethod
                + " (mutation: "
                + mutation.getFile()
                + ")");
        if (precheck.isOverLong()) {
          ...
          if (info.isExpectedStepsMet()) {
            ...
            /* filling up trace */
            MuRegressionUtils.fillMuBkpJavaFilePath(
                trace, mutation.getFile().getAbsolutePath(), mutation.getMutatedClass());
            Regression.fillMissingInfo(trace, testcaseConfig);
            muTrace.setTrace(
                new TraceExecutionInfo(
                    precheck, trace, executor.getTraceExecFilePath(), precheckPath));
          }
        }
      } else {
        System.out.println(
            "FAIL TO KILL: " + testMethod + " (mutation: " + mutation.getFile() + ")");
      }
  ...
  private MutationTrace generateMutationTrace(
      AppJavaClassPath testcaseConfig, AnalysisTestcaseParams params, SingleMutation mutation)
      throws Exception {
    ICompilationUnit iunit =
        JavaUtil.findNonCacheICompilationUnitInProject(
            mutation.getMutatedClass(), params.getProjectName());
    ...
    String targetFolder =
        IResourceUtils.getAbsolutePathOsStr(iunit.getJavaProject().getOutputLocation());
    ...
    String bkOrgClassFilePath =
        ClassUtils.getClassFilePath(params.getAnalysisOutputFolder(), mutatedClassSimpleName);
    ...
    try {
      ...
      /* generate trace */
      MutationTrace mutateInfo =
          executeTestcaseWithMutation(testcaseConfig, params.getAnalysisParams(), mutation);
      bkMutatedClassFilePath =
          ClassUtils.getClassFilePath(mutation.getMutationOutputFolder(), mutatedClassSimpleName);
      ...
      if (bkMutatedClassFilePath != null) {
        params.setBkClassFiles(
            new BackupClassFiles(classFilePath, bkOrgClassFilePath, bkMutatedClassFilePath));
      }
      /* revert */
  ...
  private List<ClassLocation> findMutationLocation(
      String junitClassName, List<ClassLocation> executingStatements, AppJavaClassPath appPath) {
    List<ClassLocation> locations = new ArrayList<>();
    for (ClassLocation point : executingStatements) {
      ...
      ClassLocation location =
          new ClassLocation(point.getClassCanonicalName(), null, point.getLineNo());
  ...
  private List<SingleMutation> mutate(Trace correctTrace, AnalysisTestcaseParams params) {
    ...
    List<ClassLocation> muLocations =
        findMutationLocation(params.getJunitClassName(), executingStatements, testcaseConfig);
    List<ClassLocation> staticCandidates =
        findStaticMutationLocation(params.getJunitClassName(), muLocations, testcaseConfig);
    ...
    Mutator mutator =
        new Mutator(
            sourceFolderPath,
            params.getAnalysisOutputFolder(),
            params.getAnalysisParams().getMuTotal());
    MutationVisitor visitor =
        new TraceMutationVisitor(params.getAnalysisParams().getMutationTypes());
    ...
    List<SingleMutation> result =
        SingleMutation.from(mutations, params.getJunitClassName(), params.getTestMethod());
    return result;
  }

  private void filterLocationsInTestPackage(
      String testSrcFolderPath, List<ClassLocation> locationList) {
