...
public class MuRegressionRetriever extends RegressionRetriever {
  ...
  public MuRegression retrieveRegression(String projectName, String bugID) throws SQLException {
    ...
    try {
      ...
      Regression regression =
          retrieveRegressionInfo(regressionId, buggyTraceId, correctTraceId, conn, closables);
  ...
  /** object[3] [0]: original code [1]: mutation code [2]: mutation class name */
  private Object[] loadMutationFile(
      int orgTraceId, int muTraceId, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
    String matchList = StringUtils.join(",", orgTraceId, muTraceId);
    PreparedStatement ps =
        conn.prepareStatement(
            String.format("SELECT * FROM MutationFile WHERE trace_id IN (%s)", matchList));
  ...
  public List<String> loadMuBugIds(
      String projectName, Connection conn, List<AutoCloseable> closables) throws SQLException {
    PreparedStatement ps =
        conn.prepareStatement(
            "SELECT bug_id FROM Regression WHERE project_name=? AND LOWER(bug_id) LIKE 'mu-%'");
    int idx = 1;
