...
public class LineVisitor extends ByteCodeVisitor {
    ...
    if (code != null) {
      for (ClassLocation location : inputLocations) {
        List<InstructionHandle> correspondingInstructions =
            findCorrespondingInstructions(location.getLineNumber(), code);
        ...
        for (InstructionHandle insHandle : writeInsns) {
          ...
          for (CFGNode postDominatee : cfgNode.getPostDominatee()) {
            ...
            ClassLocation newLoc =
                new ClassLocation(
                    location.getClassCanonicalName(), location.getMethodSign(), instructionLine);
            result.add(newLoc);
          }
        }
      }
  ...
  private List<InstructionHandle> containWriteInstructions(
      List<InstructionHandle> correspondingInstructions) {
