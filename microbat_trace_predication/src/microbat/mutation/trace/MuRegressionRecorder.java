...
public class MuRegressionRecorder extends RegressionRecorder {
  ...
  public void record(
    ...
    try {
      ...
      int correctTraceId =
          insertTrace(correctTrace, projectName, null, tc[0], tc[1], conn, closables);
      int regressionId =
          insertRegression(
              projectName, bugId, trial, buggyTraceId, correctTraceId, conn, closables);
  ...
  private void insertMutationFile(
      String mutationClassName,
      Map<Integer, String> mutationFiles,
      Connection conn,
      List<AutoCloseable> closables)
      throws SQLException {
    String sql =
        "INSERT INTO MutationFile (trace_id, mutation_file, mutation_class_name)"
            + " VALUES (?,?,?)";
