...
public class MutationEvaluator {
  ...
  public MutationExecutionResult runSingleMutationTrial(
      MutationCase mutationCase, IMutationExperimentMonitor monitor) {
    ...
    TraceExecutionInfo correctTrace =
        restoreTrace(
            mutationCase.getCorrectTraceExec(),
            mutationCase.getCorrectPrecheckPath(),
            mutationCase.getTestcaseParams().getProjectName(),
            MuRegressionUtils.createProjectClassPath(params),
            false);
    result.setCorrectTrace(correctTrace.getTrace());

    TraceExecutionInfo mutationTrace =
        restoreTrace(
            mutationCase.getBugTraceExec(),
            mutationCase.getBugPrecheckPath(),
            mutationCase.getTestcaseParams().getProjectName(),
            MuRegressionUtils.createProjectClassPath(params),
            true);
    MuRegressionUtils.fillMuBkpJavaFilePath(
        mutationTrace.getTrace(), mutation.getMutationJavaFile(), mutation.getMutatedClass());
    result.setBugTrace(mutationTrace.getTrace());

    AppJavaClassPathWrapper.wrapAppClassPath(
        mutationTrace.getTrace(), correctTrace.getTrace(), params.getBkClassFiles());
        ...
        if (foundRootCause) {
          ...
          try {
            ...
            if (!trial.getDeadEndRecordList().isEmpty()) {
              ...
              //							DED datas = new TrainingDataTransfer().transfer(record,
              // trial.getBuggyTrace());
              setTestCase(datas, trial.getTestcase());
              //							new DeadEndReporter().export(datas.getAllData(), params.getProjectName(),
              // muBugId);
              new DeadEndCSVWriter(params.getProjectOutputFolder())
                  .export(datas.getAllData(), params.getProjectName(), muBugId);
            }
          } catch (Throwable e) {
            e.printStackTrace();
          } finally {
  ...
  private CheckResult checkRootCause(
    ...
    AppJavaClassPathWrapper.wrapAppClassPath(
        killingMutatantTrace, correctTrace, params.getBkClassFiles());
  ...
  protected TraceExecutionInfo restoreTrace(
      String execPath,
      String precheckPath,
      String projectName,
      AppJavaClassPath appJavaClassPath,
      boolean isMutationTrace) {
