...
public class TestCaseAnalyzerOld {
  ...
  public boolean runEvaluationForSingleTestCase(
    ...
    if (checker.isPassingTest()) {
      ...
      List<ClassLocation> locationList =
          findMutationLocation(junitClassName, executingStatements, testcaseConfig);
  ...
  private EvaluationInfo runEvaluationForSingleTrial(
    ...
    try {
      ...
      if (killingMutatantTrace != null && killingMutatantTrace.size() > 1) {
        if (null == correctTrace) {
          ...
          correctTrace =
              new TraceModelConstructor()
                  .constructTraceModel(
                      testcaseConfig, executingStatements, executionOrderList, stepNum, true, true);
        }
        ...
        PairList pairList =
            traceMatcher.matchTraceNodePair(killingMutatantTrace, correctTrace, null);
        ...
        DiffMatcher diffMatcher =
            new MuDiffMatcher(
                testcaseConfig.getPreferences().get(SOURCE_FOLDER_KEY),
                orgFilePath,
                mutationFilePath);
        ...
        if (trials0 != null) {
          ...
          dbRecorder.record(
              ...
              correctTrace,
              pairList,
              iunit.getJavaProject().getElementName(),
              getMuBugId(mutationFilePath));
        }
        // TODO
  ...
  private List<ClassLocation> findMutationLocation(
      String junitClassName, List<BreakPoint> executingStatements, AppJavaClassPath appPath) {
