...
public class RunSingleMutationHandler extends AbstractHandler {
  ...
  private void runSingleMutation(IProgressMonitor monitor) throws IOException {
    MutationRegressionSettings settings =
        MutationRegressionPreference.getMutationRegressionSettings();
    AnalysisParams analysisParams = new AnalysisParams(settings);
    String projectFolder =
        IProjectUtils.getProjectFolder(
            JavaUtil.getSpecificJavaProjectInWorkspace(settings.getTargetProject()));
    MutationCase mutationCase =
        MutationCase.load(
            settings.getTargetProject(),
            settings.getBugId(),
            settings.getMutationOutputSpace(),
            analysisParams,
            projectFolder);
    MutationEvaluator evaluator = new MutationEvaluator();
    MutationExperimentMonitor mutationMonitor =
        new MutationExperimentMonitor(monitor, settings.getTargetProject(), analysisParams);
  ...
  public static MuDiffMatcher initDiffMatcher(MutationCase mutationCase) {
    ...
    String testFolder =
        IResourceUtils.getRelativeSourceFolderPath(
            orgPath, targetProject, mutationCase.getTestcaseParams().getJunitClassName());
  ...
  public static PairList buildPairList(
      Trace correctTrace, Trace buggyTrace, DiffMatcher diffMatcher) {
    /* PairList */
    System.out.println(
