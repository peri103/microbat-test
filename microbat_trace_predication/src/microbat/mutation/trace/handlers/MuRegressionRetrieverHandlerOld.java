  ...
  public Object execute(ExecutionEvent event) throws ExecutionException {
    String projectName =
        Activator.getDefault()
            .getPreferenceStore()
            .getString(MutationRegressionPreference.TARGET_PROJECT_KEY);
    String bugId =
        Activator.getDefault()
            .getPreferenceStore()
            .getString(MutationRegressionPreference.BUG_ID_KEY);

    try {
      MuRegression muRegression =
          new MuRegressionRetriever().retrieveRegression(projectName, bugId);
      ...
      String muPath =
          IResourceUtils.getFolderPath(projectName, "microbat/mutation/" + bugId + "/bug");
      //			String orgPath = IResourceUtils.getFolderPath(projectName, "microbat/mutation/" + bugId +
      // "/fix");
      //			String srcFolder = "src";
      String orgPath = IResourceUtils.getProjectPath(projectName);
      String srcFolder =
          IResourceUtils.getRelativeSourceFolderPath(
              orgPath, projectName, muRegression.getMutationClassName());
      String testFolder =
          IResourceUtils.getRelativeSourceFolderPath(
              orgPath, projectName, regression.getTestClass());
      String orgJFilePath =
          ClassUtils.getJFilePath(
              FileUtils.getFilePath(orgPath, srcFolder), muRegression.getMutationClassName());
      String muJFilePath =
          ClassUtils.getJFilePath(
              FileUtils.getFilePath(muPath, srcFolder), muRegression.getMutationClassName());
