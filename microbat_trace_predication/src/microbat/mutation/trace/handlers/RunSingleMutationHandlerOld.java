...
public class RunSingleMutationHandlerOld extends AbstractHandler {
          ...
          protected IStatus run(IProgressMonitor monitor) {
            ...
            try {
              ...
              MuRegression muRegression =
                  retriever.retrieveRegression(
                      targetProject, muBugId, monitor, useSliceBreaker, breakerLimit);
              ...
              String srcFolder =
                  IResourceUtils.getRelativeSourceFolderPath(
                      orgPath, targetProject, muRegression.getMutationClassName());
              String testFolder =
                  IResourceUtils.getRelativeSourceFolderPath(
                      orgPath, targetProject, regression.getTestClass());
              String orgJFilePath =
                  ClassUtils.getJFilePath(
                      FileUtils.getFilePath(orgPath, srcFolder),
              ...
              MuRegressionUtils.fillMuBkpJavaFilePath(
                  buggyTrace, muJFilePath, muRegression.getMutationClassName());
  ...
  private EmpiricalTrial simulate(
      Trace buggyTrace,
      Trace correctTrace,
      PairList pairList,
    ...
    if (rootcauseFinder.getRealRootCaseList().isEmpty()) {
      ...
      StepOperationTuple tuple =
          new StepOperationTuple(
              simulator.getObservedFault(),
              new UserFeedback(UserFeedback.UNCLEAR),
              simulator.getObservedFault(),
              DebugState.UNCLEAR);
  ...
  private PairList buildPairList(Trace correctTrace, Trace buggyTrace, DiffMatcher diffMatcher) {
    /* PairList */
    System.out.println(
        "start matching trace..., buggy trace length: "
            + buggyTrace.size()
