...
public class CsvRegenerator extends MutationEvaluator {
  ...
  public void regenerate(MutationCase mutationCase, IMutationExperimentMonitor monitor) {
    ...
    MuRegressionUtils.fillMuBkpJavaFilePath(
        mutationTrace, mutation.getMutationJavaFile(), mutation.getMutatedClass());
    ...
    PairList pairList =
        RunSingleMutationHandler.buildPairList(correctTrace, mutationTrace, diffMatcher);
    try {
      Simulator simulator =
          new Simulator(
      ...
      rootcauseFinder.checkRootCause(
          simulator.getObservedFault(), mutationTrace, correctTrace, pairList, diffMatcher);
      TraceNode rootCause =
          rootcauseFinder.retrieveRootCause(pairList, diffMatcher, mutationTrace, correctTrace);
      if (rootCause != null) {
        generator.runSimulator(
            mutation, params, mutationTrace, correctTrace, diffMatcher, pairList);
      }
    } catch (Exception e) {
