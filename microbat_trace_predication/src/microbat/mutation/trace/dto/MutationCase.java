...
public class MutationCase {
    ...
    if (isValid) {
      String validPath =
          FileUtils.getFilePath(
              mutationOutputSpace,
              "mutation",
              testcaseParams.getProjectName(),
              "validMutationCases.csv");
      storeCsv(validPath);
    }
    storeCsv(
        MuRegressionUtils.getMutationCaseFilePath(
            testcaseParams.getProjectName(), mutationOutputSpace));
  }
  ...
  public void storeCsv(String csvFilePath) {
    ...
    try {
      ...
      csvPrinter.printRecord(
          ...
          mutation.getLine(),
          getRelativePath(
              testcaseParams.getAnalysisOutputFolder(), mutation.getFile().getAbsolutePath()),
  ...
  public static MutationCase load(
      String targetProject,
      String muBugId,
      String mutationOutputSpace,
  ...
  public static List<MutationCase> loadAllMutationCases(
      String targetProject,
      String mutationOutputSpace,
      AnalysisParams analysisParams,
      String projectFolder)
  ...
  private static MutationCase toMutationCase(
      String targetProject, AnalysisParams analysisParams, String projectFolder, CSVRecord record) {
    AnalysisTestcaseParams testcaseParams =
        new AnalysisTestcaseParams(
            targetProject,
            record.get(Column.JUNIT_CLASS_NAME),
            record.get(Column.TEST_METHOD),
            analysisParams,
            projectFolder);
    ...
    mutation.setMutatedJFile(
        new File(
            getAbsolutePath(analysisOutputFolder, record.get(Column.MUTATED_JFILE_RELATIVE_PATH))));
    mutation.setMutationType(record.get(Column.MUTATION_TYPE));
    mutation.setSourceFolder(
        getAbsolutePath(testcaseParams.getProjectFolder(), record.get(Column.SOURCE_FOLDER)));
    ...
    mutationCase.correctTraceExec =
        getAbsolutePath(analysisOutputFolder, record.get(Column.CORRECT_EXEC_RELATIVE_PATH));
    mutationCase.correctPrecheckPath =
        getAbsolutePath(analysisOutputFolder, record.get(Column.CORRECT_PRECHECK_RELATIVE_PATH));
    mutationCase.bugTraceExec =
        getAbsolutePath(analysisOutputFolder, record.get(Column.BUG_EXEC_RELATIVE_PATH));
    mutationCase.bugPrecheckPath =
        getAbsolutePath(analysisOutputFolder, record.get(Column.BUG_PRECHECK_RELATIVE_PATH));
    ...
    IJavaProject project =
        JavaCore.create(JavaUtil.getSpecificJavaProjectInWorkspace(targetProject));
    BackupClassFiles backupClassFiles =
        new BackupClassFiles(
            ClassUtils.getClassFilePath(
                IProjectUtils.getTargetFolder(project), mutation.getMutatedClass()),
  ...
  public static List<CSVRecord> getRecords(String targetProject, String mutationOutputSpace)
      throws IOException {
    String mutationCasePath =
        MuRegressionUtils.getValidMutationCaseFilePath(targetProject, mutationOutputSpace);
    if (!new File(mutationCasePath).exists()) {
      mutationCasePath =
          MuRegressionUtils.getMutationCaseFilePath(targetProject, mutationOutputSpace);
    }
  ...
  public static List<String> loadAllMutationBugIds(
      String targetProject, String mutationOutputSpace) {
    try {
      List<CSVRecord> records = getRecords(targetProject, mutationOutputSpace);
