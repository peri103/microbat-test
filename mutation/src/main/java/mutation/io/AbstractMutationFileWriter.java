...
public class AbstractMutationFileWriter {
  ...
  protected String[] toString(Node node, Node orgNode) {
    ...
    if (!(orgNode instanceof BlockStmt)
        && (node instanceof BlockStmt)
        && "{".equals(lines[0])
        && "}".equals(lines[lines.length - 1])) {
  ...
  protected String extractStrAfterNode(List<?> lines, Node node) {
    return subString((String) lines.get(toFileLineIdx(node.getEndLine())), node.getEndColumn() + 1);
  }
  ...
  protected String subString(String line, int javaParserStartCol, int javaParserEndCol) {
  ...
  protected int getMappedColIdx(char[] chars, int javaParserCol, int startIdx, int startPos) {
    ...
    throw new SavRtException(
        StringUtils.spaceJoin(
            "cannot map column index between inputStream and javaparser, line = ",
