...
public class DefaultMutationVisitor extends MutationVisitor {
  ...
  public boolean mutate(IntegerLiteralExpr n) {
    ...
    Integer val =
        stringValue.startsWith("0x") ? Integer.decode(stringValue) : Integer.valueOf(stringValue);
    int newVal = val;
    if (val.intValue() == Integer.MAX_VALUE) {
      ...
      newVal =
          Randomness.nextInt(val - INT_CONST_ADJUST_HALF_VALUE, val + INT_CONST_ADJUST_HALF_VALUE);
    }
    newNode(n).getMutatedNodes().add(new IntegerLiteralExpr(String.valueOf(newVal)));
    return false;
  }
}
