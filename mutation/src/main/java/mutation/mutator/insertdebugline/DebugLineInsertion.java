...
public class DebugLineInsertion extends AbstractMutationVisitor {
  ...
  public void init(String className, ClassDescriptor classDescriptor, List<Integer> lines) {
  ...
  private void buildMapOldLineToNewLine(
      DebugLineInsertionResult result, Map<Integer, DebugLineData> mutatedLines) {
    int offset = 0;
  ...
  public boolean mutate(ReturnStmt n) {
    ...
    newNodes.add(declarationStmt(curMethod.getType(), newVarName, n.getExpr()));
    newNodes.add(returnStmt(nameExpr(newVarName)));
    Integer curLoc = getCurrentLocation(n);
    returnStmts.put(curLoc, new ReplacedLineData(curLoc, n, newNodes));
    return false;
  }
  ...
  private boolean doesReturnStmtNeedMutate(Expression returnExpr) {
    return returnExpr != null
        && !CollectionUtils.existIn(
            returnExpr.getClass(), LiteralExpr.class, NameExpr.class, ThisExpr.class);
  }
