...
public class VariableSubstitution {
  ...
  private Type searchVarTypeInMethods(
      int lineNumber, String varName, List<MethodDescriptor> methods) {
  ...
  private Type searchVarTypeInScopes(int lineNumber, String varName, List<LocalVariable> scopes) {
  ...
  public List<VariableDescriptor> findSubstitutions(String varName, int line, int col) {
     ...
     */
    if (type == null) {
      log.warn("Cannot specify type for variable with name", varName, "in", descriptor.getName());
      return result;
    }
  ...
  private static List<VariableDescriptor> findVariables(
      ClassDescriptor classDescriptor, Type type, int line, int col) {
  ...
  private static List<VariableDescriptor> findVariables(
      List<LocalVariable> scopes, Type type, int lineNumber, int column) {
    List<VariableDescriptor> result = new ArrayList<VariableDescriptor>();
    for (LocalVariable scope : scopes) {
      if (scope.containsLine(lineNumber)) {
        for (VariableDescriptor localVarInMethod : scope.getVars().values()) {
  ...
  private static boolean isVisibleAndMatchType(
      VariableDescriptor localVarInMethod, int lineNumber, int column, Type type) {
    return localVarInMethod.getPosition().declaredBefore(lineNumber, column)
        && localVarInMethod.getType().equals(type);
  }
}
