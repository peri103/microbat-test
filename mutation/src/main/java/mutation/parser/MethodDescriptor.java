...
public class MethodDescriptor {
  ...
  public void openScope(int beginLine) {
    localVars.add(new LocalVariable(beginLine, -1, new HashMap<String, VariableDescriptor>()));
  ...
  public void addLocalVar(VariableDescriptor variableDescriptor) {
    localVars.get(openScopeIdx).put(variableDescriptor.getName(), variableDescriptor);
  }

  public void addLocalVars(List<VariableDescriptor> vList) {
    for (VariableDescriptor var : vList) localVars.get(openScopeIdx).put(var.getName(), var);
  }
