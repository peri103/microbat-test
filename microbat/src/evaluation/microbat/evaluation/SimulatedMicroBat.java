...
public class SimulatedMicroBat {
  ...
  private PairList matchTraceNodePair(Trace mutatedTrace, Trace correctTrace) {
    ...
    differ.diff(
        mutatedTraceNodeWrapper,
        correctTraceNodeWrapper,
        false,
    ...
    for (GraphDiff diff : diffList) {
      if (diff.getDiffType().equals(GraphDiff.UPDATE)) {
        ...
        TraceNodePair pair =
            new TraceNodePair(wrapperBefore.getTraceNode(), wrapperAfter.getTraceNode());
  ...
  public void prepare(
    ...
    rootCause =
        findRootCause(
            mutatedLocation.getClassCanonicalName(),
            mutatedLocation.getLineNo(),
            mutatedTrace,
            getPairList());
  ...
  private TraceNode findObservedFault(List<TraceNode> wrongNodeList, PairList pairList) {
    ...
    /**
     * If the last portion of steps in trace are all wrong-path nodes, then we choose the one at the
     * beginning of this portion as the observable step.
     */
  ...
  /**
   * Adjust the effect of constraints. If last feedback is wrong-path, then this step should not be
   * a correct step.
   */
  private boolean hasConflicts(
      ArrayList<StepOperationTuple> jumpingSteps, UserFeedback currentFeedback) {
    ...
    String lastFeedbackType =
        jumpingSteps.isEmpty()
            ? null
            : jumpingSteps.get(jumpingSteps.size() - 2).getUserFeedback().getFeedbackType();

    if (lastFeedbackType != null
        && lastFeedbackType.equals(UserFeedback.WRONG_PATH)
  ...
  class Attempt {
    ...
    public boolean equals(Object obj) {
      if (obj instanceof Attempt) {
        ...
        if (this.suspiciousNodeOrder == other.suspiciousNodeOrder) {
          ...
          String thisReadVarID =
              (this.option.getReadVar() != null) ? this.option.getReadVar().getVarID() : "null";
          String thatReadVarID =
              (other.option.getReadVar() != null) ? other.option.getReadVar().getVarID() : "null";

          String thisWrittenVarID =
              (this.option.getWrittenVar() != null)
                  ? this.option.getWrittenVar().getVarID()
                  : "null";
          String thatWrittenVarID =
              (other.option.getWrittenVar() != null)
                  ? other.option.getWrittenVar().getVarID()
                  : "null";
    ...
    public int hashCode() {
      int readVarCode =
          (this.option.getReadVar() != null) ? this.option.getReadVar().getVarID().hashCode() : 0;
      int writtenVarCode =
          (this.option.getWrittenVar() != null)
  ...
  private Trial startSimulation(
    ...
    /**
     * this variable is for optimization. when a suspicious node with certain option (i.e., wrong
     * variables) is popped out of the confusing stack, it means that it is not possible to find the
     * mutated bug with this option on the suspicious node. Therefore, there is no need to try such
     * attempt again.
     */
    ...
    try {
      ...
      jumpingSteps.add(
          new StepOperationTuple(suspiciousNode, feedback, referenceNode, recommender.getState()));
      ...
      while (!isBugFound) {
        ...
        if (suspiciousNode == null && feedback.getFeedbackType().equals(UserFeedback.WRONG_PATH)) {
          ...
          jumpingSteps.add(
              new StepOperationTuple(originalSuspiciousNode, f, null, recommender.getState()));
          break;
        }
        ...
        if ((jumpingSteps.size() > mutatedTrace.size())
            ...
            || (lastNode.getOrder() == suspiciousNode.getOrder()
        ...
        /*|| cannotConverge(jumpingSteps)*/ ) {
          ...
          if (currentConfusingState != null) {
            Attempt attempt =
                new Attempt(
                    currentConfusingState.getState().getCurrentNodeOrder(),
          ...
          if (!confusingStack.isEmpty()) {
            ...
            jumpingSteps.add(
                new StepOperationTuple(
                    suspiciousNode, feedback, referenceNode, recommender.getState()));
          } else {
            break;
          }
        } else {
          ...
          if (!isBugFound) {
            ...
            jumpingSteps.add(
                new StepOperationTuple(suspiciousNode, f, null, recommender.getState()));
          }
        }
      ...
      Trial trial =
          constructTrial(
              rootCause, mutatedTrace, testCaseName, mutatedFile, isBugFound, jumpingSteps);
  ...
  private boolean canFindRecurringStep(
      StepOperationTuple checkingStep, ArrayList<StepOperationTuple> jumpingSteps, int limit) {
  ...
  private boolean isContainedInJump(
      TraceNode suspiciousNode, ArrayList<StepOperationTuple> jumpingSteps) {
  ...
  private UserFeedback operateFeedback(
    ...
    for (int i = size - 1; i >= 0; i--) {
      ...
      state.recordCheckingState(
          suspiciousNode,
          recommender,
          mutatedTrace,
          Settings.interestedVariables,
          Settings.wrongPathNodeOrder,
          Settings.potentialCorrectPatterns);

      ChosenVariableOption option = user.getOtherOptions().get(i);
      ArrayList<StepOperationTuple> clonedJumpingSteps =
          (ArrayList<StepOperationTuple>) jumpingSteps.clone();
  ...
  private Trial constructTrial(
      TraceNode rootCause,
      Trace mutatedTrace,
      String testCaseName,
      String mutatedFile,
      boolean isBugFound,
      List<StepOperationTuple> jumpingSteps) {
    ...
    for (StepOperationTuple tuple : jumpingSteps) {
      String correspondingStr =
          (tuple.getReferenceNode() == null) ? "" : tuple.getReferenceNode().toString();

      String str =
          tuple.getNode().toString()
              + ": "
              + tuple.getUserFeedback()
              + " ... "
              + correspondingStr
              + "\n";
  ...
  private List<TraceNode> findCommonNodes(
      List<TraceNode> dominatees, List<TraceNode> actualWrongNodes) {
  ...
  private TraceNode findRootCause(
      String className, int lineNo, Trace mutatedTrace, PairList pairList) {
    for (TraceNode node : mutatedTrace.getExecutionList()) {
      if (node.getDeclaringCompilationUnitName().equals(className)
          && node.getLineNumber() == lineNo) {
