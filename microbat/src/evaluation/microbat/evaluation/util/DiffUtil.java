...
public class DiffUtil {
  /**
   * compare traces directly seems not working, which may report may false positive for the
   * matching.
   *
   * @param multisetList
  ...
  public static PairList generateMatchedTraceNodeList(Trace mutatedTrace, Trace correctTrace) {
    ...
    return generateMatchedTraceNodeList(
        mutatedTraceArray, correctTraceArray, new TraceNodeComprehensiveSimilarityComparator());
  }

  public static PairList generateMatchedTraceNodeList(
      TraceNode[] mutatedTraceArray,
      TraceNode[] correctTraceArray,
    ...
    for (int i = mutatedTraceArray.length, j = correctTraceArray.length; (i > 0 && j > 0); ) {
      if (mutatedTraceArray[i - 1].hasSameLocation(correctTraceArray[j - 1])) {
        ...
        if (Math.abs(sim - increase) < 0.01) {
          TraceNodePair pair =
              new TraceNodePair(mutatedTraceArray[i - 1], correctTraceArray[j - 1]);
          pairList.add(pair);
  ...
  private static double[][] buildScoreTable(
      TraceNode[] nodeList1, TraceNode[] nodeList2, TraceNodeSimilarityComparator comparator) {
    double[][] similarityTable = new double[nodeList1.length + 1][nodeList2.length + 1];
    for (int i = 0; i < nodeList1.length + 1; i++) similarityTable[i][0] = 0;
    for (int j = 0; j < nodeList2.length + 1; j++) similarityTable[0][j] = 0;
        ...
        if (nodeList1[i - 1].hasSameLocation(nodeList2[j - 1])) {
          ...
          double value =
              similarityTable[i - 1][j - 1]
                  + comparator.compute(nodeList1[i - 1], nodeList2[j - 1]);
          similarityTable[i][j] =
              getLargestValue(value, similarityTable[i - 1][j], similarityTable[i][j - 1]);
        } else {
          similarityTable[i][j] =
