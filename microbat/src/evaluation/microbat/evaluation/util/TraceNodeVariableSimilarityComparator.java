...
/**
 * This class is used to compare the variable difference between two trace node. If two trace nodes
 * in the same source code location have the same read and written variables, their commonality, or
 * similarity, is 1. Otherwise, a similarity value ranging from 0 to 1, indicating how similar they
 * are, will be returned.
 *
 * @author Yun Lin
 */
public class TraceNodeVariableSimilarityComparator implements TraceNodeSimilarityComparator {
  ...
  public double compute(TraceNode traceNode1, TraceNode traceNode2) {
    ...
    if (traceNode1.hasSameLocation(traceNode2)) {
      double commonReadVarWithSameValue =
          findCommonVarWithSameValue(
              traceNode1.getReadVariables(), traceNode1, traceNode2.getReadVariables(), traceNode2);
      double commonWrittenVarWithSameValue =
          findCommonVarWithSameValue(
              traceNode1.getWrittenVariables(),
              traceNode1,
              traceNode2.getWrittenVariables(),
              traceNode2);

      int totalVars =
          traceNode1.getReadVariables().size()
              + traceNode1.getWrittenVariables().size()
              + traceNode2.getWrittenVariables().size()
              + traceNode2.getReadVariables().size();
      ...
      if (totalVars == 0) {
        ...
        simVarScore =
            (2 * (double) commonReadVarWithSameValue + 2 * commonWrittenVarWithSameValue)
                / totalVars;
      }
  ...
  private double findCommonVarWithSameValue(
      List<VarValue> variables1, TraceNode node1, List<VarValue> variables2, TraceNode node2) {
    double common = 0;
  ...
  //	private boolean isExactlyTheSame(List<VarValue> var1s, TraceNode node1, List<VarValue> var2s,
  // TraceNode node2) {
      ...
      if (isSameName) {
        ...
        if (!(var1 instanceof ReferenceValue) && !(var2 instanceof ReferenceValue)) {
          ...
          if (refVar1.getChildren() != null && refVar2.getChildren() != null) {
            ...
            differ.diff(
                var1, var2, true, sortedMatcher, /*EvaluationSettings.variableComparisonDepth*/ -1);
