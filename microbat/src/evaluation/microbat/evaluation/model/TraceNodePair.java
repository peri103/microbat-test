...
public class TraceNodePair {
  ...
  public String toString() {
    return "TraceNodePair [originalNode="
        + originalNode
        + ", mutatedNode="
        + mutatedNode
        + ", isExactlySame="
  ...
  public List<VarValue> findSingleWrongWrittenVarID(Trace trace) {
    ...
    for (VarValue mutatedWrittenVar : mutatedNode.getWrittenVariables()) {
      List<VarValue> mutatedVarList =
          findCorrespondingVarWithDifferentValue(
              mutatedWrittenVar,
              originalNode.getWrittenVariables(),
              mutatedNode.getWrittenVariables(),
              trace,
              Variable.WRITTEN);
  ...
  public List<VarValue> findSingleWrongReadVar(Trace mutatedTrace) {
    ...
    for (VarValue mutatedReadVar : mutatedNode.getReadVariables()) {
      List<VarValue> mutatedVarList =
          findCorrespondingVarWithDifferentValue(
              mutatedReadVar,
              originalNode.getReadVariables(),
              mutatedNode.getReadVariables(),
              mutatedTrace,
              Variable.READ);
      if (!mutatedVarList.isEmpty()) {
        for (VarValue value : mutatedVarList) {
   ...
   *
   * <p>However, it is possible that it returns a list of two variable values. Such a case happen
   * when the mutated variable and original variable are synonymous reference variable, e.g., obj.
   * They are different because some of their attributes or elements are different, e.g., obj.x. In
   * this case, the simulated user does not know whether the reference variable itself (e.g., obj)
   * is wrong, or its attribute or element (e.g., obj.x) is wrong. Therefore, we return both case
   * for the simulated debugging. In above case, the first element in the list is the reference
   * variable value, and the second one is the wrong attribute variable value.
   *
   * @param mutatedVar
   * @param originalList
   * @param mutatedList
   * @return
   */
  private List<VarValue> findCorrespondingVarWithDifferentValue(
      VarValue mutatedVar,
      List<VarValue> originalList,
      List<VarValue> mutatedList,
      Trace mutatedTrace,
      String RW) {
    ...
    if (mutatedVar instanceof VirtualValue) {
      for (VarValue originalVar : originalList) {
        if (originalVar instanceof VirtualValue
            && mutatedVar.getType().equals(originalVar.getType())) {
          ...
          if (mutatedIndex == originalIndex) {
            if (((VirtualValue) mutatedVar).isOfPrimitiveType()
                || mutatedVar.isDefinedToStringMethod()) {
        ...
        if (originalVar instanceof ReferenceValue) {
          if (mutatedVar.getVarName().equals(originalVar.getVarName())) {
            ...
            if (mutatedRefVar.getChildren() != null && originalRefVar.getChildren() != null) {
              ...
              SortedGraphMatcher sortedMatcher =
                  new SortedGraphMatcher(
                      new Comparator<GraphNode>() {
                        ...
                        public int compare(GraphNode o1, GraphNode o2) {
                          if (o1 instanceof VarValue && o2 instanceof VarValue) {
                            return ((VarValue) o1)
                                .getVarName()
                                .compareTo(((VarValue) o2).getVarName());
                          }
                          return 0;
                        }
                      });
              differ.diff(
                  ...
                  originalRefVar,
                  true,
                  sortedMatcher, /*EvaluationSettings.variableComparisonDepth*/
                  -1);
                ...
                if (diff.getDiffType().equals(GraphDiff.UPDATE)) {
                  ...
                  if (!mutatedSubVarValue.equals(mutatedVar)) {
                    ...
                    if (!varID.contains(":") && !varID.contains(VirtualVar.VIRTUAL_PREFIX)) {
                      String order =
                          mutatedTrace.findDefiningNodeOrder(
                              Variable.READ,
                              mutatedNode,
                              varID,
                              mutatedSubVarValue.getAliasVarID());
                      varID = varID + ":" + order;
                    }
