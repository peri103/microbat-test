...
public class TestCaseAnalyzer {
  ...
  public void test() {
    String str =
        "C:\\Users\\YUNLIN~1\\AppData\\Local\\Temp\\mutatedSource8245811234241496344\\47_25_1\\Main.java";
  ...
  private void cleanClassInTestPackage(String sourceFolderPath, List<ClassLocation> locationList) {
  ...
  public void runEvaluationForSingleTrial(
      String testClassName,
      String testMethodName,
      String mutationFile,
      double unclearRate,
      boolean enableLoopInference,
      boolean isReuseTrace,
    ...
    if ((cachedMutatedTrace == null || cachedCorrectTrace == null) || !isReuseTrace) {
      ...
      correctTrace =
          new TraceModelConstructor()
              .constructTraceModel(
                  testcaseConfig, executingStatements, executionOrderList, checker.getStepNum());
    ...
    microbat.prepare(
        killingMutatantTrace, correctTrace, mutatedLocation, testcaseName, mutationFile);
    Trial trial;
    try {
      trial =
          microbat.detectMutatedBug(
  ...
  private void visualize(
      final Trace killingMutatantTrace, final Trace correctTrace, final PairList pairList) {
  ...
  public void runEvaluation(
      ...
      boolean isLimitTrialNum,
      IgnoredTestCaseFiles ignoredTestCaseFiles,
      ParsedTrials parsedTrials,
      int trialNumPerTestCase,
      double[] unclearRates,
      int optionSearchLimit,
      ...
      if (javaElement instanceof IPackageFragment) {
        ...
        if (!testingMethods.isEmpty()) {
          ...
          for (MethodDeclaration testingMethod : testingMethods) {
            ...
            try {
              runEvaluationForSingleTestCase(
                  className,
                  methodName,
                  reporter,
                  isLimitTrialNum,
  ...
  public boolean runEvaluationForSingleTestCase(
      String className,
      String methodName,
      ExcelReporter reporter,
      boolean isLimitTrialNum,
  ...
  private EvaluationInfo runEvaluationForSingleTrial(
      String tobeMutatedClass,
      File mutationFile,
      AppJavaClassPath testcaseConfig,
      int line,
      String testCaseName,
      Trace correctTrace,
      List<BreakPoint> executingStatements,
    ...
    try {
      ...
      if (killingMutatantTrace != null && killingMutatantTrace.size() > 1) {
        if (null == correctTrace) {
          System.out.println(
              "The correct trace of " + stepNum + " steps is to be generated for " + testCaseName);
          correctTrace =
              new TraceModelConstructor()
                  .constructTraceModel(
                      testcaseConfig, executingStatements, executionOrderList, stepNum);
        }
  ...
  private MutateInfo generateMutateTrace(
      AppJavaClassPath testcaseConfig,
    ...
    try {
      ...
      if (isKill) {
        System.out.println(
            "KILLED: Now generating trace for " + testMethod + " (mutation: " + mutatedFile + ")");
        ...
        if (checker.isOverLong()) {
          System.out.println(
              "The trace is over long for " + testMethod + " (mutation: " + mutatedFile + ")");
          ...
          killingMutantTrace =
              constructor.constructTraceModel(
                  testcaseConfig, executingStatements, executionOrderList, checker.getStepNum());
  ...
  private List<ClassLocation> findMutationLocation(
      List<BreakPoint> executingStatements, AppJavaClassPath appPath) {
    ...
    for (BreakPoint point : executingStatements) {
      ...
      if (cu != null && !JTestUtil.isInTestCase(className, appPath)) {
        ...
        if (checker.isLoopInsider()) {
          ClassLocation location = new ClassLocation(className, null, point.getLineNumber());
          locations.add(location);
        }
      }
    }

    List<ClassLocation> invocationMutationPoints =
        findInvocationMutations(executingStatements, appPath);
  ...
  private List<ClassLocation> findInvocationMutations(
      final List<BreakPoint> executingStatements, final AppJavaClassPath appPath) {
            ...
            public boolean visit(MethodInvocation invocation) {
              ...
              if (startLine == point.getLineNumber()) {
                ...
                if (checker.isValid) {
                  ...
                  CompilationUnit cuOfDec =
                      JavaUtil.findCompilationUnitInProject(type.getQualifiedName(), appPath);
                  ...
                  MethodDeclaration declaration =
                      (MethodDeclaration) cuOfDec.findDeclaringNode(declarationBinding);
                  if (declaration == null) {
                    return false;
                  }
                  ...
                  for (BreakPoint point : executingStatements) {
                    if (startMDLine < point.getLineNumber() && point.getLineNumber() < endMDLine) {
                      ClassLocation location =
                          new ClassLocation(
                              point.getDeclaringCompilationUnitName(), null, point.getLineNumber());
  ...
  private AppJavaClassPath createProjectClassPath(String className, String methodName) {
    ...
    String junit5RunnerPath =
        junitDir + File.separator + "org.junit.platform.runner_1.8.1.v20211018-1956.jar";
