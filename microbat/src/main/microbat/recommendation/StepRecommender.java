...
public class StepRecommender {
  ...
  public TraceNode recommendNode(Trace trace, TraceNode currentNode, UserFeedback userFeedback) {
    ...
    if (feedbackType.equals(UserFeedback.UNCLEAR)) {
      ...
      if (state == DebugState.SLICING
          ...
          || state == DebugState.DETAIL_INSPECT) {
      ...
      if (earliestVisitedNode == null) {
        ...
        && (feedbackType.equals(UserFeedback.WRONG_VARIABLE_VALUE)
            || feedbackType.equals(UserFeedback.WRONG_PATH))) {
  ...
  private TraceNode findClearerNodeInBetween(
      Trace trace, TraceNode currentNode, TraceNode earliestVisitedUnclearNode) {
    ...
    if (latestWrongNode != null) {
      ...
      while (iter.hasNext()) {
        ...
        boolean shouldRemove =
            dominator.getOrder() < currentNode.getOrder()
  ...
  private TraceNode recommendSuspiciousNode(
      Trace trace, TraceNode currentNode, UserFeedback userFeedBack) {
  ...
  private TraceNode handleBinarySearchState(
      Trace trace, TraceNode currentNode, UserFeedback userFeedback) {
    TraceNode suspiciousNode = null;
    ...
    if (isOverSkipping) {
      ...
      if (startNode != null) {
        ...
        if (pattern != null) {
          ...
          System.err.println(
              "error in binary search for "
                  + fakePath.getLineTrace()
  ...
  private TraceNode handleDataSlicing(
      Trace trace, TraceNode currentNode, UserFeedback userFeedback) {
    ...
    //			if(parentVarID != null){
    //				suspiciousNode = trace.getLatestProducerBySimpleVarIDForm(currentNode.getOrder(),
    // parentVarID);
    //			}
    //			else{
    //				return currentNode;
    ...
    //		}
    //
    //		if(this.isEnableLoopInference() && isPathInPattern && !shouldStopOnCheckedNode(currentNode,
    // path)){
    //			state = DebugState.SKIP;
    //
    ...
    //
    //				suspiciousNode =
    // Settings.potentialCorrectPatterns.inferNextSuspiciousNode(oldSusiciousNode);
   ...
   * @return
   */
  private TraceNode handleSimpleInferenceState(
      Trace trace, TraceNode currentNode, UserFeedback userFeedBack) {
    ...
    if (userFeedBack.getFeedbackType().equals(UserFeedback.WRONG_PATH)) {
      ...
      InspectingRange inspectingRange =
          new InspectingRange(currentNode, latestCause.getBuggyNode());
    ...
    if (!suspiciousNode.hasChecked()) {
      ...
      if (suspiciousNode.isAllReadWrittenVarCorrect(false)) {
        ...
        if (pattern != null) {
          ...
          Variable compatibaleVariable =
              findCompatibleReadVariable(causingVariable, suspiciousNode);
  ...
  private TraceNode handleDetailInspectingState(
      Trace trace, TraceNode currentNode, UserFeedback userFeedBack) {
  ...
  /**
   * Find the variable causing the jump of label path of the <code>pattern</code>, noted as <code>
   * var</code>, then try finding the dominator of the <code>oldSusiciousNode</code> by following
   * the same dominance chain with regard to <code>var</code>. The dominator of <code>
   * oldSusiciousNode</code> on <code>var</code> is the new suspicious node. <br>
   * <br>
   ...
   * @return
   */
  public TraceNode findNextSuspiciousNodeByPattern(
      PotentialCorrectPattern pattern, TraceNode oldSusiciousNode) {
    PathInstance labelPath = pattern.getLabelInstance();
