...
public class BugInferer {
  public Bug infer(TraceNode currentNode, StepRecommender recommender) {
    ...
    if (startNode == null) {
      ...
      if (startNode != null) {
        ...
        List<PathInstance> labelPaths =
            Settings.potentialCorrectPatterns.findSimilarIterationPath(path);
  ...
  private List<TraceNode> pathDiff(PathInstance path, PathInstance labelPath) {
    ...
    while (startNode.getOrder() <= path.getEndNode().getOrder()) {
      SourceLine line =
          path
          .new SourceLine(
              startNode.getClassCanonicalName(),
  ...
  private TraceNode findIterationHead(
    ...
    SourceLine line =
        new PathInstance()
        .new SourceLine(
            currentNode.getClassCanonicalName(),
            currentNode.getLineNumber(),
            currentNode.isLoopCondition());
    List<PotentialCorrectPattern> patterns =
        potentialCorrectPatterns.findPatternsHavingSameEndNodeWith(line);
