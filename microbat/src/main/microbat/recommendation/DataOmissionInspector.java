...
public class DataOmissionInspector extends DetailInspector {
  ...
  public List<TraceNode> analyze(VarValue wrongValue) {
    ...
    Map<MethodNode, List<InstructionHandle>> seeds =
        seedFinder.findSeedStatemets(wrongValue, start, end);
    if (seeds == null) {
      return new ArrayList<>();
    }
    ...
    List<BreakPoint> controlPoints =
        controlFinder.findSeedControlDominators(trace.allLocations(), seeds);
  ...
  private boolean isExecutedPotentialSeeds(
      TraceNode controlDominator, Map<MethodNode, List<InstructionHandle>> seeds, int endOrder) {
