...
public class SeedStatementFinder {
  ...
  public Map<MethodNode, List<InstructionHandle>> findSeedStatemets(
      VarValue specificVar, TraceNode start, TraceNode end) {
  ...
  private Map<MethodNode, List<InstructionHandle>> matchVarLocally(
      TraceNode start, VarValue specificVar) {
    Map<MethodNode, List<InstructionHandle>> map = new HashMap<>();

    Trace trace = start.getTrace();
    Method method =
        new CallGraph(trace.getAppJavaClassPath(), trace.getIncludedLibraryClasses())
            .findByteCodeMethod(start.getBreakPoint());

    MethodNode methodNode =
        new MethodNode(trace.getAppJavaClassPath().getClassLoader(), start.getMethodSign(), method);
  ...
  private Map<MethodNode, List<InstructionHandle>> matchVarWithCallGraph(
      Trace trace, VarValue specificVar) {
