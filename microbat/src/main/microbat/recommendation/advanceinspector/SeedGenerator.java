...
public class SeedGenerator {
  ...
  public Map<String, List<Unit>> findSeeds(
      VarValue var, UnitGraph contextGraph, boolean reanalysisPointsTo) {
  ...
  private Map<String, List<Unit>> findCorrespondingDefsInAllMethods(
      VarValue var, RelationChain chain) {
  ...
  private List<Unit> checkLinearizedDefs(
      SootMethod method, VarValue varValue, RelationChain chain) {
  ...
  //	public List<Unit> findSeeds(VarValue var, UnitGraph graph, List<Unit>
  ...
  /**
   * reverse traverse the CFG, if a CFG node defines the {@code varValue}, then the algorithm stops
   * propagating. Otherwise, the algorithm traverse back to the entry CFG node to find the possible
   * CFG node (i.e., unit) defining the {@code varValue}.
   *
   * @param varValue
   * @param currentUnit
   * @param seeds
   ...
   * @param parsedUnits
   */
  private List<Unit> findVarDefsInGraph(
      VarValue varValue,
      Unit currentUnit,
      List<Unit> seeds,
      RelationChain chain,
   ...
   * @return
   */
  private boolean checkUnitDefiningInterestingVariable(
      Unit unit, UnitGraph graph, List<Unit> seeds, VarValue varValue, RelationChain chain) {
   ...
   * which decompose an expression into a series of three-address instructions. It means if there is
   * a very complicated expression like ``a.k[b].c'', Soot must decompose it into a series of simple
   * variable definitions.
   *
   * <p>Therefore, we start from {@code definingUnit} which defines the interested variable {@code
   * varValue}, and go back in CFG to search the instructions of linearization. Thus, we can
   * recursively match the parents of interested variable to these linearized instruction to see
   * whether two chains of variables are matched. For example, obj.array[1] shall be matched to
   ...
   * @return
   */
  private boolean resursiveMatch(
      Value val, Unit definingUnit, UnitGraph graph, RelationChain chain) {
