...
public class PotentialCorrectPatternList {
  ...
  public List<PathInstance> findSimilarIterationPath(PathInstance path) {
    ...
    for (PotentialCorrectPattern pattern : this.patterns.values()) {
      ...
      SourceLine patternStartLine =
          path
          .new SourceLine(
              labelPath.getStartNode().getClassCanonicalName(),
              labelPath.getStartNode().getLineNumber(),
              labelPath.getStartNode().isLoopCondition());
      SourceLine patternEndLine =
          path
          .new SourceLine(
              labelPath.getEndNode().getClassCanonicalName(),
              labelPath.getEndNode().getLineNumber(),
              labelPath.getEndNode().isLoopCondition());

      SourceLine pathStartLine =
          path
          .new SourceLine(
              path.getStartNode().getClassCanonicalName(),
              path.getStartNode().getLineNumber(),
              path.getStartNode().isLoopCondition());
      SourceLine pathEndLine =
          path
          .new SourceLine(
              path.getEndNode().getClassCanonicalName(),
              path.getEndNode().getLineNumber(),
              path.getEndNode().isLoopCondition());

      if (patternStartLine.equals(pathStartLine) && patternEndLine.equals(pathEndLine)) {
        list.add(labelPath);
      }
    }
  ...
  public List<PotentialCorrectPattern> findPatternsHavingSameEndNodeWith(SourceLine line) {
    ...
    for (PotentialCorrectPattern pattern : this.patterns.values()) {
      ...
      SourceLine endLine =
          new PathInstance()
          .new SourceLine(
              node.getClassCanonicalName(), node.getLineNumber(), node.isLoopCondition());
  ...
  /**
   * Given existing suspicious node, I try to infer a possible wrong-value variable, <code>var
   * </code>, on this suspicious node. If there exists pattern which can cover the path consisting
   * of the data dominant node of suspicious node on <code>var</code>, Then the dominance node will
   * be returned as the next suspicious node.<br>
   * <br>
   * Such an inference is based on abduction. A path covered by a pattern means that a similar path
   * has been feedbacked as non-buggy. Therefore, we skip such kind of path to speed up jumping on
   * the trace.
   *
  ...
  public TraceNode inferNextSuspiciousNode(TraceNode existingSusiciousNode) {
    SourceLine line =
        ...
        .new SourceLine(
            ...
            existingSusiciousNode.getLineNumber(),
            existingSusiciousNode.isLoopCondition());
    ...
    for (PotentialCorrectPattern pattern : patterns) {
      ...
      TraceNode dataDominator =
          findDataDominatorOnCertainVariable(existingSusiciousNode, causingVar);
  ...
  private TraceNode findDataDominatorOnCertainVariable(
      TraceNode existingSusiciousNode, Variable causingVar) {
