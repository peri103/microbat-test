...
public class Trace {
  ...
  /**
   * This variable is to trace whether the variables in different lines are the same local variable.
   */
  ...
  /**
   * This variable indicate the time of user ask for recommendation, in addition, the check time is
   * also used to specify the time of a variable marked as "incorrect". Note that, newer variables
   * has more importance in the trace.
   */
   ...
   * steps. key is the variable ID, and value is the entry containing all the steps reading/writing
   * the corresponding variable.
   */
  @Deprecated private Map<String, StepVariableRelationEntry> stepVariableTable = new HashMap<>();
  ...
  public void constructLoopParentRelation() {
    ...
    for (TraceNode node : this.executionList) {
      ...
      /** if out of the scope the loop parent, pop this step decide the influential loop parent. */
      if (!loopParentStack.isEmpty()) {
        ...
        while (!isLoopParentContainDirectlyOrIndirectly(currentLoopParent, node)
            ...
            || (node.getLineNumber() == currentLoopParent.getLineNumber()
                && loopParentHaveNotLoopChildOfSomeInvocationParentOfNode(currentLoopParent, node))
      ...
      /**
       * connect loop parent-child relation this step decide the direct loop child for the
       * influential loop parent in the peek of the stack
       */
      if (!loopParentStack.isEmpty()) {
  ...
  private boolean isLoopParentContainDirectlyOrIndirectly(
      TraceNode currentLoopParent, TraceNode node) {
  ...
  private void testAndAppendTryCatchControlFlow(TraceNode node) {
    ...
    CompilationUnit cu =
        JavaUtil.findCompilationUnitInProject(
            node.getDeclaringCompilationUnitName(), this.appJavaClassPath);
    if (clause == null) {
      if (cu == null) {
        cu =
            JavaUtil.findCompiltionUnitBySourcePath(
                node.getBreakPoint().getFullJavaFilePath(), node.getDeclaringCompilationUnitName());
      }
  ...
  private boolean isClauseContainScope(
      TraceNode node, CompilationUnit cu, CatchClause clause, TraceNode existingControlDom) {
  ...
  //
  ////			if(producers.size() > 1){
  ////				System.err.println("there are more than one producer for variable " +
  // entry.getAliasVariables());
  ////			}
  //
  //			if(!producers.isEmpty()){
   ...
   * defining v.
   *
   * <p>if we are finding defining step of a written variable: (1) if it is not a field/index of an
   ...
   * @return
   */
  public String findDefiningNodeOrder(
      String accessType, TraceNode currentNode, String varID, String aliasVarID) {
  ...
  /**
   * Note that, if a variable is a primitive type, I cannot retrieve its heap address, therefore, I
   * use the static approach to uniquely identify a variable, i.e., variable ID. Please refer to
   * {@link microbat.model.variable.Variable#varID} for details. <br>
   * <br>
  ...
  public TraceNode findMostSupiciousNode(AttributionVar var) {
    ...
    for (TraceNode node : this.executionList) {
      if (suspiciousNode == null) {
        ...
        //					System.out.println(node.getOrder() + "(" + var.getVarID() + "):" +
        // node.getSuspicousScore(var));
        //				}
      }
    }
  ...
  public LoopSequence findLoopRangeOf(TraceNode currentNode) {
    ...
    if (controlDominator != null) {
      ...
      LoopSequence loopSequence =
          new LoopSequence(range.get(0).getOrder(), range.get(range.size() - 1).getOrder());
   ...
   * @return
   */
  private List<TraceNode> extendLoopRange(
      List<TraceNode> allControlDominatees, TraceNode controlLoopDominator) {
