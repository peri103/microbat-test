...
public class PathInstance {
  ...
  public ArrayList<SourceLine> reducePattern(ArrayList<SourceLine> instanceLineTrace) {
    ...
    while (!containsNoDuplication(pattern)) {
      ...
      /**
       * mark which range should be removed and which source line should be deheaded (mark it as no
       * a loop head).
       */
      ...
      /**
       * If the source line of a step is deheaded (set isLoopHead as false), then all the steps on
       * the same source line should be deheaded as well.
       *
       * <p>In addition, if the merge does not happen, we should dehead all the steps so that the
  ...
  private boolean containsNoDuplication(ArrayList<SourceLine> instanceLineTrace) {
