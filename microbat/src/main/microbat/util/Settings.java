...
public class Settings {
   ...
   * interested library code. However, it is possible that the interested library code is called by
   * some normal library code, of course, these normal library code is called by application code.
   * In such case, we will not capture the data/control dependencies of the library code called by
   * normal library code. By this means, many of the library variables will not be visited, saving
   * the time for recording the trace.
   *
   * <p>We use this field to decide whether such an optimization is enabled or not.
   */
      ...
      try {
        projectName =
            Activator.getDefault()
                .getPreferenceStore()
                .getString(MicrobatPreference.TARGET_PORJECT);
        launchClass =
            Activator.getDefault().getPreferenceStore().getString(MicrobatPreference.LANUCH_CLASS);
        testMethod =
            Activator.getDefault().getPreferenceStore().getString(MicrobatPreference.TEST_METHOD);
        String isRecord =
            Activator.getDefault()
                .getPreferenceStore()
                .getString(MicrobatPreference.RECORD_SNAPSHORT);
        isRecordSnapshot = isRecord.equals("true");
        String isApplyAInspector =
            Activator.getDefault()
                .getPreferenceStore()
                .getString(MicrobatPreference.APPLY_ADVANCE_INSPECTOR);
        isApplyAdvancedInspector = isApplyAInspector.equals("true");
        String isRuntest =
            Activator.getDefault().getPreferenceStore().getString(MicrobatPreference.RUN_TEST);
        isRunTest = isRuntest.equals("true");
        String limitNumString =
            Activator.getDefault().getPreferenceStore().getString(MicrobatPreference.STEP_LIMIT);
        stepLimit = Integer.valueOf(limitNumString);
        if (stepLimit == 0) {
          stepLimit = 5000;
        }
        String varLayerString =
            Activator.getDefault()
                .getPreferenceStore()
                .getString(MicrobatPreference.VARIABLE_LAYER);
        variableLayer = Integer.valueOf(varLayerString);

        String applyRecordingOptimization =
            Activator.getDefault()
                .getPreferenceStore()
                .getString(MicrobatPreference.RECORDING_OPTIMIZATION);
        applyLibraryOptimization = applyRecordingOptimization.equals("true");

        String supportConcurrentTraceString =
            Activator.getDefault()
                .getPreferenceStore()
                .getString(MicrobatPreference.SUPPORT_CONCURRENT_TRACE);
        supportConcurrentTrace = supportConcurrentTraceString.equals("true");

        String runWithDebugModeString =
            Activator.getDefault()
                .getPreferenceStore()
                .getString(MicrobatPreference.RUN_WITH_DEBUG_MODE);
  ...
  public static PotentialCorrectPatternList potentialCorrectPatterns =
      new PotentialCorrectPatternList();
