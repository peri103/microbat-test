...
/**
 * TraceUtil provides helper function to analyze the trace
 *
 * @author WYK
 */
public class TraceUtil {
  ...
  /**
   * Perform dynamic slicing on the given trace based on the given target variables. <br>
   * <br>
   * It serves as a filter filtering out the node that does not contribute to the target variables
   * <br>
   * <br>
   * Procedures: <br>
   * 1. It will first search the nodes containing the target variables starting from the end. <br>
   * 2. For each node containing the target variables, perform breath first search based on data and
   * control relation to search the related node until it cannot go further. <br>
   * <br>
   * Note that this function will ignore the throwing exception node.
   *
   ...
   */
  public static List<TraceNode> dyanmicSlice(
      final Trace trace, final Collection<VarValue> targets) {
    UniquePriorityQueue<TraceNode> toVisitNodes =
        new UniquePriorityQueue<>(
    ...
    switch (feedback.getType()) {
      ...
      default:
        throw new RuntimeException(
            Log.genMsg("TraceUtil", "Unhandled feedback type: " + feedback.getType().name()));
    }
    nextNodes.removeIf(nextNode -> nextNode == null);
    return nextNodes;
  }

  public static TraceNode findNextNode(
      final TraceNode node, final UserFeedback feedback, final Trace trace) {
  ...
  public static Set<TraceNode> findAllNextNodes(
      final TraceNode node, final Collection<UserFeedback> feedbacks, final Trace trace) {
  ...
  public static int relationDistance(
      final TraceNode node1, final TraceNode node2, final Trace trace, final int disLimit) {
