...
public class MicroBatUtil {
  ...
  public static AppJavaClassPath constructClassPaths(String projectName) {
    ...
    // projectPath.indexOf(File.separator+Settings.projectName));
  ...
  public static void setSystemJars(AppJavaClassPath appClassPath) {
    ...
    if (!jar.exists()) {
      ...
      if (mircobatFeature.length > 0) {
        String jarPath =
            mircobatFeature[0].getAbsolutePath()
                + File.separator
                + "junit_lib"
                + File.separator
                + "instrumentator.jar";
  ...
  public static String getDefinedJavaHomeFolder() {
    String javaHomePath =
        Activator.getDefault().getPreferenceStore().getString(MicrobatPreference.JAVA7HOME_PATH);
  ...
  public static String combineTraceNodeExpression(
      String className, int lineNumber, long duration, int order, double suspiciousness) {
    String exp =
        className
            + " line:"
            + lineNumber
  ...
  /**
   * For string1: a b c d string2: a f c d The result is a c d
   *
   * @param nodeList1
   * @param nodeList2
   * @param comparator
  ...
  public static Object[] generateCommonNodeList(Object[] nodeList1, Object[] nodeList2) {
    ...
    for (int k = commonLength - 1, i = nodeList1.length, j = nodeList2.length; (i > 0 && j > 0); ) {
      if (nodeList1[i - 1].equals(nodeList2[j - 1])) {
        ...
        if (commonLengthTable[i - 1][j] >= commonLengthTable[i][j - 1]) i--;
        else j--;
      }
    }
  ...
  public static int[][] buildLeveshteinTable(Object[] nodeList1, Object[] nodeList2) {
    ...
    for (int i = 0; i < nodeList1.length + 1; i++) commonLengthTable[i][0] = 0;
    for (int j = 0; j < nodeList2.length + 1; j++) commonLengthTable[0][j] = 0;
   ...
   * @param curTrace
   */
  public static void assignWrittenIdentifier(
      List<VarValue> retrievedChildren, TraceNode currentNode) {
  ...
  //
  //
  //	private static boolean isEqualRange(VarValue var1, InterestedVariable v1, VarValue var2,
  // InterestedVariable v2) {
  ...
  //				boolean isRootVar1InScope = lvs.getStartLine() <= v1.getLineNumber() && lvs.getEndLine() >=
  // v1.getLineNumber();
  //				boolean isRootVar2InScope = lvs.getStartLine() <= v2.getLineNumber() && lvs.getEndLine() >=
  // v2.getLineNumber();
  //
  ...
  public static String generateTraceDir(String projectName, String bugID) {
    String traceFolder =
        sav.common.core.utils.FileUtils.getFilePath(
            MicroBatUtil.getTraceFolder(), projectName, bugID);
