...
public class StepVectorizer {
  ...
  public StepVector vectorize(int order) {
    ...
    // set previous
    stepVector.stepInPrev =
        Optional.ofNullable(targetStep.getStepInPrevious()).map(step -> step.getOrder()).orElse(0);
    stepVector.stepOverPrev =
        Optional.ofNullable(targetStep.getStepOverPrevious())
            .map(step -> step.getOrder())
            .orElse(0);
    // set next
    stepVector.stepOverNext =
        Optional.ofNullable(targetStep.getStepOverNext()).map(step -> step.getOrder()).orElse(0);
    stepVector.stepInNext =
        Optional.ofNullable(targetStep.getStepInNext()).map(step -> step.getOrder()).orElse(0);
    // set control flow
    stepVector.controlFlow.add(
        Optional.ofNullable(targetStep.getControlDominator()).map(s -> s.getOrder()).orElse(0));
    // set data flow
