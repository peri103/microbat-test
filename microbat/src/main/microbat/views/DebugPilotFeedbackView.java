...
public class DebugPilotFeedbackView extends ViewPart {
  ...
  public static final String ID = "microbat.view.debugPilotFeedback";
  ...
  protected void createReadVariablesViewer(final Composite parent) {
    ...
    this.readVariableViewer.addDragSupport(
        this.operations, this.transferTypes, this.createDragSourceAdapter(this.readVariableViewer));
  ...
  protected void createWrittenVariableViewer(final Composite parent) {
    ...
    //		this.writtenVariableViewer.addDragSupport(this.operations, this.transferTypes,
    // this.createDragSourceAdapter(this.writtenVariableViewer));
  ...
  protected void createOutputGroup(final Composite parent) {
    ...
    this.shouldNotExecuteButton.addSelectionListener(
        new SelectionListener() {
          @Override
          public void widgetSelected(SelectionEvent e) {
            DebugPilotInfo.getInstance()
                .setOutputNodeWrongBranch(shouldNotExecuteButton.getSelection());
          }
    ...
    this.wrongOutputTreeViewer.addDropSupport(
        this.operations,
        this.transferTypes,
        new ViewerDropAdapter(wrongOutputTreeViewer) {
          @Override
          public boolean performDrop(Object data) {
            if (data instanceof IStructuredSelection selection) {
              wrongOutputTreeViewer.setInput(
                  Arrays.stream(selection.toArray())
                      .collect(Collectors.toCollection(ArrayList::new)));
              VarValue[] selectedVar =
                  Arrays.stream(selection.toArray())
                      .map(
                          obj -> {
                            return (VarValue) obj;
                          })
                      .toArray(VarValue[]::new);
    ...
    this.wrongOutputTreeViewer.addDoubleClickListener(
  ...
  protected void createAvailiableFeedbacksTable(final Composite parent) {
    ...
    //		this.availableFeedbackViewer = TableViewer.newCheckList(variableForm, SWT.H_SCROLL |
    // SWT.V_SCROLL| SWT.FULL_SELECTION | SWT.CHECK | SWT.MULTI);
    this.availableFeedbackViewer =
        new TableViewer(variableForm, SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.FULL_SELECTION);
  ...
  protected void createGiveFeedbackGroup(final Composite parent) {
    ...
    this.feedbackButton.addSelectionListener(
  ...
  protected void refreshWrittenVariableViewer() {
    this.writtenVariableViewer.setContentProvider(
        new WrittenVariableContentProvider(this.currentNode));
  ...
  protected void refreshRelatedVariableGroup() {
    ...
    this.relatedVariablesViewer.setContentProvider(
        new WrittenVariableContentProvider(invokeParent));
    this.relatedVariablesViewer.setInput(
        invokeParent == null ? null : invokeParent.getWrittenVariables());
  ...
  protected void refreshAvailableFeedbackViewer() {
    ...
    this.typeViewerColumn.setLabelProvider(
    ...
    this.varViewerColumn.setLabelProvider(
        new ColumnLabelProvider() {
          @Override
          public String getText(Object element) {
            if (element instanceof UserFeedback userFeedback) {
              //    				if (userFeedback.getFeedbackType().equals(UserFeedback.WRONG_VARIABLE_VALUE)
              // || userFeedback.getFeedbackType().equals(UserFeedback.CORRECT_VARIABLE_VALUE)) {
              if (userFeedback.getFeedbackType().equals(UserFeedback.WRONG_VARIABLE_VALUE)) {
    ...
    this.varValueViewerColumn.setLabelProvider(
        new ColumnLabelProvider() {
          @Override
          public String getText(Object element) {
            if (element instanceof UserFeedback userFeedback) {
    ...
    this.nextNodeViewerColumn.setLabelProvider(
        new nextNodeButtonLabelProvider(this.currentNode, this.trace));
