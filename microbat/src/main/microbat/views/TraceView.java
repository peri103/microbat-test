...
public class TraceView extends ViewPart {
  ...
  /**
   * indicate whether the program state should be refreshed when a trace node is selected
   * programmatically.
   */
  ...
  protected void markJavaEditor(TraceNode node) {
    ...
    try {
      ...
      AnnotationModel annotationModel =
          (AnnotationModel)
              sourceEditor.getDocumentProvider().getAnnotationModel(sourceEditor.getEditorInput());
      ...
      ReferenceAnnotation annotation =
          new ReferenceAnnotation(false, "Please check the status of this line");
    ...
    public String getText(Object element) {
      if (element instanceof TraceNode) {
        ...
        String message =
            order
                + ". "
                + MicroBatUtil.combineTraceNodeExpression(
                    className, lineNumber, duration, predOrder, node.getSuspicousness());
        return message;
      }
