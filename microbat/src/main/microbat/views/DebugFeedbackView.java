...
public class DebugFeedbackView extends ViewPart {
  ...
  /**
   * Here, the 0th element indicates input; 1st element indicates output; and 2nd element indicates
   * state.
   */
  ...
  public void refresh(TraceNode node) {
    ...
    if (this.readVariableTreeViewer.getCheckedElements() != null
        && this.readVariableTreeViewer.getCheckedElements().length != 0) {
      readVar = (VarValue) this.readVariableTreeViewer.getCheckedElements()[0];
    }

    VarValue writtenVar = null;
    if (this.writtenVariableTreeViewer.getCheckedElements() != null
        && this.writtenVariableTreeViewer.getCheckedElements().length != 0) {
  ...
  protected void createSubmitGroup(Composite parent) {
    ...
    correctButton.addMouseListener(
        new MouseListener() {
    ...
    wrongVarButton.addMouseListener(
        new MouseListener() {
          public void mouseUp(MouseEvent e) {}
    ...
    wrongPathButton.addMouseListener(
        new MouseListener() {
          public void mouseUp(MouseEvent e) {}
    ...
    unclearButton.addMouseListener(
        new MouseListener() {
          public void mouseUp(MouseEvent e) {}
  ...
  protected boolean isValidToInferBugType() {
    if (currentNode != null) {
      ...
      //			boolean flag1 = currentNode.getReadVarCorrectness(Settings.interestedVariables,
      // true)==TraceNode.READ_VARS_CORRECT &&
      //					currentNode.getWittenVarCorrectness(Settings.interestedVariables,
      // true)==TraceNode.WRITTEN_VARS_INCORRECT;
  ...
  class FeedbackSubmitListener implements MouseListener {
    ...
    private void openBugFoundDialog() {
      MessageBox box = new MessageBox(PlatformUI.getWorkbench().getDisplay().getActiveShell());
      box.setMessage(
          "Correct read variable with incorrect written variable, You have found the bug!");
      box.open();
    }
      ...
      Display.getDefault()
          .asyncExec(
              new Runnable() {
                ...
                public void run() {
                  Status status =
                      new Status(IStatus.ERROR, "My Plug-in ID", 0, "Conflict Choice", null);
    ...
    private boolean isValidForRecommendation() {
      int readVarCorrectness =
          currentNode.getReadVarCorrectness(Settings.interestedVariables, true);
      int writtenVarCorrectness =
          currentNode.getWittenVarCorrectness(Settings.interestedVariables, true);
      ...
      //			if(existWrittenVariable && existReadVariable &&
      // writtenVarCorrectness==TraceNode.WRITTEN_VARS_INCORRECT
      //					&& readVarCorrectness==TraceNode.READ_VARS_CORRECT){
      //				openBugFoundDialog();
      //				return false;
      //			}
      //			else if(existWrittenVariable && existReadVariable &&
      // writtenVarCorrectness==TraceNode.WRITTEN_VARS_CORRECT
      ...
      //				return false;
      //			}
      //			else if(((existWrittenVariable &&
      // writtenVarCorrectness==TraceNode.WRITTEN_VARS_INCORRECT) ||
      //					(existReadVariable && readVarCorrectness==TraceNode.READ_VARS_INCORRECT))
      //					&& feedbackType.equals(UserFeedback.CORRECT)){
      //				String message = "Some variables are marked incorrect, but your feedback is marked
      // correct (\"Yes\" choice), "
      //						+ "are you really sure?";
      //				openReconfirmDialog(message);
      //				return false;
      //			}
      else if (lastFeedbackType != null
          && lastFeedbackType.equals(UserFeedback.WRONG_PATH)
          && feedbackType.equals(UserFeedback.CORRECT)) {
        String message =
            "The lastest node has wrong path, but you now tell me that no variable "
    ...
    public void mouseDown(MouseEvent e) {
      if (feedback == null) {
        ...
        Job job =
            new Job("searching for suspicious step...") {
              @Override
              protected IStatus run(IProgressMonitor monitor) {
                ...
                if (isValidForRecommendation) {
                  ...
                  if (feedback.getFeedbackType().equals(UserFeedback.WRONG_VARIABLE_VALUE)
                      && feedback.getOption() == null) {
                    List<VarValue> list =
                        currentNode.getWrongReadVars(Settings.interestedVariables);
                    if (!list.isEmpty()) {
                  ...
                  final TraceNode suspiciousNode =
                      recommender.recommendNode(trace, currentNode, feedback);
                  lastFeedbackType = feedbackType;

                  if (recommender.getState() == DebugState.BINARY_SEARCH
                      || recommender.getState() == DebugState.SKIP) {
      ...
      if (ele instanceof GraphDiff) {
        ...
        if (element == null) {
          ...
          if (element instanceof ArrayValue) {
            ...
            switch (columnIndex) {
              case 0:
                return "array[" + value.getComponentType() + "]";
              case 1:
