...
public class TraceConstructor {
  ...
  public void test() throws SavException {
    String jarPath =
        "E:/lyly/Projects/microbat/master/microbat_instrumentator/src/resources/instrumentator.jar";
    ...
    config.addClasspath(
        "E:/lyly/eclipse-java-mars-clean/eclipse/dropins/junit_lib/org.hamcrest.core.jar");
  ...
  public Trace getTrace(AppJavaClassPath appClassPath) throws Exception {
    String instrumentationJarPath =
        IResourceUtils.getResourceAbsolutePath(Activator.PLUGIN_ID, "lib") + "/instrumentation.jar";
    ...
    agentRunner.addAgentParam(
        "entry_point",
        ClassUtils.toClassMethodStr(
            appClassPath.getOptionalTestClass(), appClassPath.getOptionalTestMethod()));
  ...
  public int runClosure11() throws Exception {
    ...
    config.addClasspath(
        "E:/linyun/bug_repo/Closure/11/fix/lib/rhino/testsrc/org/mozilla/javascript/tests/commonjs/module/modules.jar");
    ...
    agentRunner.addAgentParams(
        AgentParams.OPT_LOG,
        Arrays.asList(LogType.printProgress, LogType.info, LogType.debug, LogType.error));
    long start = System.currentTimeMillis();
  ...
  public void runMockito6() throws Exception {
    ...
    config.addClasspath(
        "E:/linyun/bug_repo/Mockito/6/bug/lib/run/com.springsource.org.hamcrest.core-1.1.0.jar");
    config.addClasspath("E:/linyun/bug_repo/Mockito/6/bug/lib/run/objenesis-2.1.jar");
    config.addClasspath(
        "E:/linyun/bug_repo/Mockito/6/bug/lib/sources/cglib-and-asm-1.0-sources.jar");
    config.addClasspath(
        "E:/linyun/bug_repo/Mockito/6/bug/lib/sources/com.springsource.org.hamcrest.core-1.1.0-sources.jar");
    ...
    agentRunner.addAgentParams(
        ...
        Arrays.asList(LogType.printProgress, LogType.info, LogType.debug, LogType.error));
    long start = System.currentTimeMillis();
    agentRunner.runWithDumpFileOption(
        "E:/lyly/eclipse-java-mars-clean/eclipse-for-unmodified-code/trace/Mockito/6/fix.exec");
    Trace trace = agentRunner.getMainTrace();
    System.out.println("isTestSuccessful? " + agentRunner.isTestSuccessful());
    System.out.println("testFailureMessage: " + agentRunner.getTestFailureMessage());
  ...
  public void loadTrace() {
    ...
    RunningInfo runningInfo = RunningInfo.readFromFile("E:/lyly/bug.exec");
    System.out.println(timer.getResult());
    //		int maxR = -1;
    //		int maxW = -1;
    //		for (TraceNode node : runningInfo.getTrace().getExecutionList()) {
    //			System.out.print(node.getOrder() + ": ");
    //			System.out.println("R= " + node.getReadVariables().size() + ", W= " +
    // node.getWrittenVariables().size());
