  ...
  public InputParameterVector(final String typeDescriptor) {
    ...
    for (String type : types) {
      if (type.startsWith("[L")) {
        ...
        //				int idx = LibraryClassDetector.isLibClass(type) ? ParameterVector.LIB_OBJ_IDX :
        // ParameterVector.SELF_DEFINED_OBJ_IDX;
        ...
        if (idx < 0) {
          throw new RuntimeException(
              "[InputParameterVector] Cannot get type idx: " + typeDescriptor);
        }
        this.set(ParameterVector.ARRAY_OFFSET + idx);
      } else {
        // Primitive type
