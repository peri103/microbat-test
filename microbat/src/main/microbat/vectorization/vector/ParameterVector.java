  ...
  protected List<String> splitInputTypeDescriptor(final String descriptor) {
    ...
    for (int idx = 0; idx < descriptor.length(); idx++) {
      ...
      if (character == 'L') {
        ...
        // Handle the case for high dimensional array, we treat high dimension array same way as one
        // dimension array
        int startIdx = idx;
        ...
        if (nextChar == 'L') {
          ...
          if (!(caturedStr.startsWith("[") && caturedStr.endsWith(";"))) {
            throw new RuntimeException(
                "[InputParameterVector]: Wrong type capturiong for array of object");
          }
        } else {
          caturedStr = descriptor.substring(startIdx, idx + 1);
        }
