...
/**
 * The retriever works on any DBMS since DBMS-specific details are handled by DBService and
 * connection factory
 *
 * @author dingyuchen
 */
public class TraceRetrieverImpl implements TraceRetriever {
  ...
  private static final String GET_STEPS = "SELECT s.* FROM Step s WHERE s.trace_id=?";
  ...
  private List<TraceNode> getSteps(Trace trace) throws SQLException {
    ...
    for (int i = 0; i < total; i++) {
      TraceNode node =
          new LazyTraceNode(null, null, i + 1, trace, step -> this.loadRWVars(step, traceId));
      allSteps.add(node);
    }
    ...
    try {
      ...
      PreparedStatement ps =
          conn.prepareStatement(
              "SELECT read_vars, written_vars from Step where step_order = ? AND trace_id = ?");
      ...
      System.out.println(
          String.format(
              ...
              loadVarStep, traceId, step.getOrder()));
      throw e;
    } finally {
      DbService.closeDb(conn, closables);
      this.closables = new ArrayList<>();
   ...
   * @throws SQLException
   */
  private int countNumberOfStep(String traceId, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
  ...
  private void loadLocations(
      Map<String, List<TraceNode>> locIdStepMap, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
