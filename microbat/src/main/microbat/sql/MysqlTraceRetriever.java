...
public class MysqlTraceRetriever extends DbService {
  ...
  public int getLatestTrace(String projectName) throws SQLException {
    ...
    try {
      ...
      PreparedStatement ps = conn.prepareStatement("SELECT MAX(trace_id) FROM Trace");
  ...
  protected Trace loadTrace(int traceId, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
  ...
  private List<Object[]> loadStepVariableRelation(
      int traceId, Connection conn, List<AutoCloseable> closables) throws SQLException {
  ...
  private List<TraceNode> loadSteps(
      int traceId, Connection conn, List<AutoCloseable> closables, Trace trace)
      throws SQLException {
    ...
    while (rs.next()) {
      ...
      try {
        ...
        System.out.println(
            String.format(
                "%s: Xml error at step: [trace_id, order] = [%d, %d]",
                loadVarStep, traceId, order));
        throw e;
      }
    }
    rs.close();
  ...
  private void loadLocations(
      Map<Integer, TraceNode> locIdStepMap, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
    ...
    /* control scope */
    Map<Integer, ControlScope> controlScopeMap =
        loadControlScopes(locationSet, matchList, conn, closables);
    /* loop scope */
    ...
    PreparedStatement ps =
        conn.prepareStatement(
            String.format(
                "SELECT location_id,class_name,line_number,is_conditional,is_return FROM Location"
  ...
  private Map<Integer, ControlScope> loadControlScopes(
      Set<Integer> locationSet, String matchList, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
    ...
    PreparedStatement ps =
        conn.prepareStatement(
            String.format(
                "SELECT location_id, class_name, line_number, is_loop FROM ControlScope WHERE"
                    + " location_id IN (%s)",
                matchList));
    ResultSet rs = ps.executeQuery();
    closables.add(ps);
  ...
  private Map<Integer, SourceScope> loadLoopScope(
      Set<Integer> locationSet, String matchList, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
    if (matchList.isEmpty()) {
      return Collections.EMPTY_MAP;
    ...
    PreparedStatement ps =
        conn.prepareStatement(
            String.format(
                "SELECT location_id, class_name, start_line, end_line FROM LoopScope WHERE"
                    + " location_id IN (%s)",
                matchList));
    ResultSet rs = ps.executeQuery();
    closables.add(ps);
    ...
    while (rs.next()) {
      SourceScope scope =
          new SourceScope(
              rs.getString("class_name"), rs.getInt("start_line"), rs.getInt("end_line"));
  ...
  private TraceNode getRelNode(List<TraceNode> allSteps, ResultSet rs, String colName)
      throws SQLException {
    int relNodeOrder = rs.getInt(colName);
    if (!rs.wasNull()) {
      if (relNodeOrder > allSteps.size()) {
        System.err.println(
            String.format("index out of bound: size=%d, idx=%d", allSteps.size(), relNodeOrder));
        return null;
      }
