...
public class DebugPilotPreference extends PreferencePage implements IWorkbenchPreferencePage {
  ...
  // Paths.get("user_behavior_log.txt").toAbsolutePath().toString();
  ...
  public void init(IWorkbench workbench) {
    try {
      this.propagatorType =
          PropagatorType.valueOf(
              Activator.getDefault()
                  .getPreferenceStore()
                  .getString(DebugPilotPreference.PROPAGATOR_TYPE_KEY));
      this.pathFinderType =
          PathFinderType.valueOf(
              Activator.getDefault()
                  .getPreferenceStore()
                  .getString(DebugPilotPreference.PATHFINDER_TYPE_KEY));
      this.rootCauseLocatorType =
          RootCauseLocatorType.valueOf(
              Activator.getDefault()
                  .getPreferenceStore()
                  .getString(DebugPilotPreference.ROOT_CAUSE_LOCATOR_TYPE_KEY));
      //			this.logPath =
      // Activator.getDefault().getPreferenceStore().getString(DebugPilotPreference.LOG_PATH_KEY);
  ...
  public boolean performOk() {
    ...
    this.rootCauseLocatorType =
        RootCauseLocatorType.valueOf(this.rootCauseLocatorTypeCombo.getText());
  ...
  protected void storeData() {
    ...
    preferences.put(
        DebugPilotPreference.ROOT_CAUSE_LOCATOR_TYPE_KEY, this.rootCauseLocatorType.name());
    //		preferences.put(DebugPilotPreference.LOG_PATH_KEY, this.logPath);
    Activator.getDefault()
        .getPreferenceStore()
        .putValue(DebugPilotPreference.PROPAGATOR_TYPE_KEY, this.propagatorType.name());
    Activator.getDefault()
        .getPreferenceStore()
        .putValue(DebugPilotPreference.PATHFINDER_TYPE_KEY, this.pathFinderType.name());
    Activator.getDefault()
        .getPreferenceStore()
        .putValue(
            DebugPilotPreference.ROOT_CAUSE_LOCATOR_TYPE_KEY, this.rootCauseLocatorType.name());
    //		Activator.getDefault().getPreferenceStore().putValue(DebugPilotPreference.LOG_PATH_KEY,
    // this.logPath);
  }
  ...
  protected void createPropagatorSetting(final Composite parent) {
    ...
    final String[] selectionsName =
        Stream.of(PropagatorType.values()).map(Enum::name).toArray(String[]::new);
  ...
  protected void createPathFinderSetting(final Composite parent) {
    ...
    final String[] selectionsName =
        Stream.of(PathFinderType.values()).map(Enum::name).toArray(String[]::new);
  ...
  protected void createRootCauseLocatorSetting(final Composite parent) {
    ...
    final String[] selectionsNam =
        Stream.of(RootCauseLocatorType.values()).map(Enum::name).toArray(String[]::new);
