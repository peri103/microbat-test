...
public class BPVariableRetriever {
  ...
  public List<BreakPoint> parsingBreakPoints(
      AppJavaClassPath appClassPath, boolean isOptimizeByteCodeCompilation) throws Exception {
    ...
    try {
      ...
      /**
       * when evaluation does not change line number, so we can keep the cache to speed up the
       * progress
       */
      ...
      for (String className : class2PointMap.keySet()) {
        ...
        LineNumberVisitor visitor =
            new LineNumberVisitor(class2PointMap.get(className), appClassPath);
