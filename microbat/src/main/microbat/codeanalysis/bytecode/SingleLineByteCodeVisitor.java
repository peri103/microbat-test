...
public class SingleLineByteCodeVisitor extends ByteCodeVisitor {
  ...
  public SingleLineByteCodeVisitor(
      int lineNumber, String className, AppJavaClassPath appJavaClassPath) {
    super();
    ...
    for (InstructionHandle handle : list) {
      if (handle.getInstruction() instanceof ReturnInstruction) {
        List<InstructionHandle> previousInstructions =
            findPreviousInstructions(lineNumber, handle.getPosition(), method.getCode());
  ...
  private Variable parseReturnVariableName(
      Method method, List<InstructionHandle> previousInstructions) {
  ...
  protected List<InstructionHandle> findPreviousInstructions(
      int lineNumber, long offset, Code code) {
  ...
  protected VarOp parseReadWrittenVariable(
      InstructionHandle insHandle, Method method, AppJavaClassPath appJavaClassPath) {
    Code code = method.getCode();
    ...
    if (insHandle.getInstruction() instanceof FieldInstruction) {
      ...
      List<InstructionHandle> previousInstructions =
          findPreviousInstructions(insHandle.getPosition(), code);
   ...
   * instructions popping an array out of frame stack. If we detect k such instructions, the name of
   * variable handled by k-th instruction from AINS should be the name we need. We record k in a
   * local variable <code>popArrayTime</code>.
   *
   * @param method
    ...
    for (int i = previousInstructions.size() - 1; i >= 0; i--) {
      ...
      if (ins instanceof FieldInstruction) {
        ...
        LocalVariable localVariable =
            code.getLocalVariableTable().getLocalVariable(localVarIndex, insHandle.getPosition());
        ...
        if (type.contains("[")) {
          ...
          int varIndex = lIns.getIndex();
          LocalVariable lVar =
              method.getLocalVariableTable().getLocalVariable(varIndex, insHandle.getPosition());
          if (lVar != null) {
