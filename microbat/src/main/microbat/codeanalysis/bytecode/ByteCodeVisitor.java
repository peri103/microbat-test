...
public class ByteCodeVisitor extends EmptyVisitor {
  ...
  protected void parseReadWrittenVariable(
      BreakPoint point,
    ...
    CompilationUnit cu =
        JavaUtil.findCompilationUnitInProject(
            point.getDeclaringCompilationUnitName(), appJavaClassPath);
      ...
      if (insHandle.getInstruction() instanceof FieldInstruction) {
        ...
        if (fullFieldName != null) {
          ...
          if (!fullFieldName.contains("$")) {
            /**
             * The reason for additionally use ReadFileRetriever and WrittenFieldRetriever is that I
             * want to get "a.attr1.attr2" instead of "attr2".
             */
            if (rw) {
              ReadFieldRetriever rfRetriever =
                  new ReadFieldRetriever(cu, point.getLineNumber(), fullFieldName);
              cu.accept(rfRetriever);
              if (rfRetriever.fullFieldName != null) {
                fullFieldName = rfRetriever.fullFieldName;
              }
            } else {
              WrittenFieldRetriever wfRetriever =
                  new WrittenFieldRetriever(cu, point.getLineNumber(), fullFieldName);
        ...
        LocalVariable variable =
            code.getLocalVariableTable().getLocalVariable(lIns.getIndex(), insHandle.getPosition());
        if (variable == null) {
        ...
        if (variable != null) {
          if (variable.getName().equals("this")
              && !isThisAppearOnRightHandSide(point, appJavaClassPath)) {
            continue;
          }
        ...
        if (insHandle.getInstruction().getName().toLowerCase().contains("load")) {
          ReadArrayElementRetriever raeRetriever =
              new ReadArrayElementRetriever(cu, point.getLineNumber(), typeName);
          ...
          WrittenArrayElementRetriever waeRetriever =
              new WrittenArrayElementRetriever(cu, point.getLineNumber(), typeName);
  ...
  private void setConditionalScope(
      InstructionHandle handle, BreakPoint point, CFG cfg, Code code, CompilationUnit cu) {
  ...
  private ClassLocation transferToLocation(
      InstructionHandle insHandle, Code code, BreakPoint point) {
   ...
   * position of this instruction will be smaller than the start PC of this local variable. In other
   * words, the instruction is out of the scope of this local variable, which may cause me to
   * miss-find the correct variable. The diff will be between 1 or 2.
   *
   * <p>In order to address this issue, I searched all the variables in local variable table and
   ...
   * @return
   */
  protected LocalVariable findOptimalVariable(
      InstructionHandle insHandle, int variableIndex, LocalVariableTable localVariableTable) {
