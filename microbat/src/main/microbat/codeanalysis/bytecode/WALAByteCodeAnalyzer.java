...
public class WALAByteCodeAnalyzer {
  ...
  public List<BreakPoint> parsingBreakPoints(AppJavaClassPath appClassPath) throws Exception {
    ...
    Iterable<Entrypoint> entrypoints =
        makeEntrypoints(scope.getApplicationLoader(), cha, entryPoints);
    AnalysisOptions options = new AnalysisOptions(scope, entrypoints);

    //		CallGraphBuilder builder = Util.makeZeroOneCFABuilder(options, new AnalysisCache(), cha,
    // scope);
    CallGraphBuilder builder = Util.makeZeroCFABuilder(options, new AnalysisCache(), cha, scope);
    //		CallGraphBuilder builder = Util.makeNCFABuilder(3, options, new AnalysisCache(), cha,
    // scope);
    //		CallGraphBuilder builder = Util.makeVanillaNCFABuilder(1, options, new AnalysisCache(), cha,
    // scope);
  ...
  public static List<BreakPoint> parseLanuchPoints(AppJavaClassPath appClassPath) {
    ...
    if (appClassPath.getOptionalTestClass() != null) {
      ...
      if (mainMethod != null) {
        ...
        BreakPoint point =
            new BreakPoint(
                launchClass, launchClass, cu.getLineNumber(mainMethod.getStartPosition()));
        point.setMethodSign(methodSignature);
        points.add(point);
      }
    }
  ...
  public List<BreakPoint> slice(AppJavaClassPath appClassPath, List<BreakPoint> breakpoints)
    ...
    CallGraphBuilder builder =
        Util.makeVanillaNCFABuilder(1, options, new AnalysisCache(), cha, scope);
    ...
    //		SDG sdg = new SDG(cg, builder.getPointerAnalysis(), DataDependenceOptions.NO_BASE_PTRS,
    // ControlDependenceOptions.NONE);

    //		Collection<Statement> computeBackwardSlice = new CISlicer(cg, builder.getPointerAnalysis(),
  ...
  private int getStatementLineNumber(ShrikeCTMethod method, StatementWithInstructionIndex stmt)
  ...
  public List<BreakPoint> toBreakpoints(Collection<Statement> slice)
      throws SavException, InvalidClassFileException {
    Map<String, BreakPoint> bkpSet = new HashMap<String, BreakPoint>();

    for (Statement s : slice) {
      if (s instanceof StatementWithInstructionIndex
          && CollectionUtils.existIn(
              s.getKind(), Kind.NORMAL, Kind.NORMAL_RET_CALLEE, Kind.NORMAL_RET_CALLER)) {
  ...
  private LocalVar generateLocalVar(
      ShrikeCTMethod method, int pc, int varIndex, String locationClass, int lineNumber) {
    ...
    try {
      ...
      Class byteCodeInfoClass =
          Class.forName("com.ibm.wala.classLoader.ShrikeBTMethod$BytecodeInfo");
  ...
  private void appendReadWritenVariable(
      BreakPoint point, ShrikeCTMethod method, IInstruction[] insList, int insIndex, IR ir)
      throws InvalidClassFileException {
    ...
    CompilationUnit cu =
        JavaUtil.findCompilationUnitInProject(point.getDeclaringCompilationUnitName(), null);
    ...
    if (ins instanceof GetInstruction) {
      ...
      if (fullFieldName == null) {
        System.err.println(
            "When parsing written field name "
                + varName
      ...
      LocalVar var =
          generateLocalVar(
              method, pc, varIndex, point.getDeclaringCompilationUnitName(), point.getLineNumber());
      ...
      /**
       * I do not know why, but for wala library, the retrieved pc cannot be used to find the
       * variable name directly. I have to try some other bcIndex to find a variable name.
       */
      String varName = null;
      ...
      if (varName == null) {
        System.err.println(
            "When parsing written variable name, I cannot achieve variable name in line "
                + lineNumber
                + " of "
      ...
      ReadArrayElementRetriever raeRetriever =
          new ReadArrayElementRetriever(cu, lineNumber, typeName);
      ...
      //				System.err.println("When parsing read array element, I cannot find specific read array
      // element in line " +
      ...
      WrittenArrayElementRetriever waeRetriever =
          new WrittenArrayElementRetriever(cu, lineNumber, typeName);
      cu.accept(waeRetriever);
      String writtenArrayElement = waeRetriever.arrayElementName;

      if (writtenArrayElement == null) {
        //				System.err.println("When parsing written array element, I cannot find specific written
        // array element in line " +
        //						lineNumber + " of " + point.getClassCanonicalName());
      } else {
  ...
  private void setConditionalScope(CompilationUnit cu, int lineNumber, BreakPoint point) {
    ...
    for (int line = conditionScope.getStartLine(); line <= conditionScope.getEndLine(); line++) {
      ClassLocation location =
          new ClassLocation(JavaUtil.getFullNameOfCompilationUnit(cu), null, line);
  ...
  private String getClassCanonicalName(IMethod method) {
    ...
    String className =
        ClassUtils.getCanonicalName(
            clazz.getPackage().toString().replace("/", "."), clazz.getClassName().toString());
  ...
  private List<BreakPoint> anlyzeBreakPointsWithDataDependencies(
      CallGraph graph, AppJavaClassPath appPath) {
    Map<String, BreakPoint> bkpSet = new HashMap<String, BreakPoint>();
  ...
  private boolean isMethodContainedInExection(IMethod method, AppJavaClassPath appPath) {
    ...
    try {
      ...
      // method.getParameterSourcePosition(method.getNumberOfParameters());
  ...
  private boolean isThePositionContainedInExecution(String className, int lineNum) {
    for (BreakPoint bp : this.executingStatements) {
      if (bp.getClassCanonicalName().equals(className) && bp.getLineNumber() == lineNum) {
        return true;
      }
    }
    return false;
  ...
  public CGNode findMethod(CallGraph cg, String className, String methodName) {
    ...
    for (Iterator<? extends CGNode> it = cg.iterator(); it.hasNext(); ) {
      ...
      if (getClassCanonicalName(method).equals(className) && method.getName().equals(a)) {
        return n;
      }
    }
  ...
  public static Iterable<Entrypoint> makeEntrypoints(
    ...
    for (BreakPoint breakpoint : breakpoints) {
      String classSignature =
          trimSignature(SignatureUtils.getSignature(breakpoint.getClassCanonicalName()));
      TypeReference typeRef =
          TypeReference.findOrCreate(loaderRef, TypeName.string2TypeName(classSignature));
