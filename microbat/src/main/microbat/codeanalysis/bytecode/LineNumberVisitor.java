      ...
      for (BreakPoint breakPoint : breakPoints) {
        ...
        if (!isStartOfClass) {
          ...
          if (!correspondingInstructions.isEmpty()) {
            String methodSig =
                breakPoint.getClassCanonicalName() + "#" + method.getName() + method.getSignature();
            if (!(methodSig.contains("class$") && breakPoint.getMethodSign() != null)) {
              breakPoint.setMethodSign(methodSig);
            }

            parseReadWrittenVariable(
                breakPoint, correspondingInstructions, code, cfg, appJavaClassPath);
          }
        } else {
