...
public class VMStarter {
  ...
  public VirtualMachine start() {
    ...
    SocketLanchingConnector0 connector =
        new SocketLanchingConnector0((VirtualMachineManagerImpl) manager);
  ...
  private Map<String, Connector.Argument> connectorArguments(
      LaunchingConnector connector, AppJavaClassPath configuration) {
    ...
    String mainValue;
    if (isRunOnTestCaseMode
        && configuration.getOptionalTestClass() != null
        && configuration.getOptionalTestMethod() != null) {
      mainValue =
          configuration.getLaunchClass()
              + " "
              + configuration.getOptionalTestClass()
              + " "
