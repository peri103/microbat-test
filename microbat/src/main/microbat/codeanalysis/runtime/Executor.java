...
public abstract class Executor {
  ...
  protected void addStartBreakPointWatch(
      EventRequestManager erm, ReferenceType refType, Range range) {
  ...
  public static String[] deriveLibExcludePatterns() {
    ...
    ExtJarPackagesContainer pkgsContainer =
        new ExtJarPackagesContainer(MicroBatUtil.getRtJarPathInDefinedJavaHome(), true);
    expandedExcludes =
        FilterUtils.deriveLibExcludePatterns(pkgsContainer, excludedLibs, includedLibs);
  ...
  protected void addStepWatch(EventRequestManager erm, ThreadReference threadReference) {
    StepRequest stepRequest =
        erm.createStepRequest(threadReference, StepRequest.STEP_LINE, StepRequest.STEP_INTO);
  ...
  protected boolean hasValidThreadName(ThreadReference thread) {
    return !thread.name().equals("main")
        && !thread.name().equals("DestroyJavaVM")
  ...
  protected Value retrieveComplicatedExpressionValue(
      String expr, VirtualMachine vm, GetFrame frameGetter) {
    List<Boolean> stepSwitch = new ArrayList<>();
    for (int i = 0; i < stepRequestList.size(); i++) {
      StepRequest stepRequest = stepRequestList.get(i);
      stepSwitch.add(stepRequest.isEnabled());
   ...
   * loaded. In order to see whether the target test case is loaded, there is specific JUnit method
   * is called. This method returns whether such the specific JUnit method has been called.
   *
   * @param declareType
   * @param methodName
   * @return
  ...
  protected boolean isTagJUnitCall(String declareType, String methodName) {
    return (declareType.contains("junit.framework.TestResult") && methodName.equals("startTest"))
