...
public class TTY implements EventNotifier {
  ...
  public void fieldWatchEvent(WatchpointEvent fwe) {
    ...
    if (fwe instanceof ModificationWatchpointEvent) {
      MessageOutput.lnprint(
          "Field access encountered before after",
          new Object[] {
            field, fwe.valueCurrent(), ((ModificationWatchpointEvent) fwe).valueToBe()
          });
    } else {
  ...
  public void exceptionEvent(ExceptionEvent ee) {
    ...
    if (catchLocation == null) {
      ...
      MessageOutput.lnprint(
          ...
          new Object[] {
            ee.exception().referenceType().name(), Commands.locationString(catchLocation)
          });
    }
  }
  ...
  private void printCurrentLocation() {
    ...
    if (frame == null) {
      ...
      if (loc.lineNumber() != -1) {
        ...
        if (line != null) {
          MessageOutput.println(
              "source line number and line", new Object[] {new Integer(loc.lineNumber()), line});
        }
      }
    }
    MessageOutput.println();
    ...
    if (!cmd.startsWith("#")) {
      ...
      if (Character.isDigit(cmd.charAt(0)) && t.hasMoreTokens()) {
        ...
        if (commandNumber < 0) {
          ...
          MessageOutput.println(
              "Command not valid until the VM is started with the run command", cmd);
        } else if (Env.connection().isOpen()
            && !Env.vm().canBeModified()
  ...
  /**
   * Try to read commands from dir/fname, unless the canonical path passed in is the same as that
   * for dir/fname. Return null if that file doesn't exist, else return the canonical path of that
   * file.
   */
  ...
  public static void main(String argv[]) throws MissingResourceException {
    ...
    MessageOutput.textResources =
        ResourceBundle.getBundle(
            "microbat.codeanalysis.runtime.jpda.tty.TTYResources", Locale.getDefault());
