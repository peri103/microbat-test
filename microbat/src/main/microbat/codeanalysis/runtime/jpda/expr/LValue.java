...
abstract class LValue {
  ...
  void setValue(Value value) throws ParseException {
    try {
      ...
      throw new ParseException(
          "Attempt to set value before " + exc.className() + " was loaded" + exc);
    }
  }
  ...
  Value interiorGetValue() throws ParseException {
    ...
    try {
      ...
      throw new ParseException(
          "Unable to complete expression. Exception " + e.exception() + " thrown");
    } catch (IncompatibleThreadStateException itse) {
      throw new ParseException(
          "Unable to complete expression. Thread " + "not suspended for method invoke");
      ...
      throw new ParseException(
          "Unable to complete expression. Method "
              + "argument type "
              + tnle.className()
              + " not yet loaded");
    }
  ...
  private static class LValueInstanceMember extends LValue {
    ...
    LValueInstanceMember(Value value, String memberName, ThreadReference thread)
        throws ParseException {
      ...
      if ((matchingField == null) && overloads.size() == 0) {
        throw new ParseException(
            "No instance field or method with the name " + memberName + " in " + refType.name());
      }
    }
  ...
  private static class LValueStaticMember extends LValue {
    ...
    LValueStaticMember(ReferenceType refType, String memberName, ThreadReference thread)
        throws ParseException {
      ...
      if ((matchingField == null) && overloads.size() == 0) {
        throw new ParseException(
            "No static field or method with the name " + memberName + " in " + refType.name());
      }
    }
  ...
  static LValue makeThisObject(
      VirtualMachine vm, ExpressionParser.GetFrame frameGetter, Token token) throws ParseException {
    if (frameGetter == null) {
  ...
  static LValue makeNewObject(
    ...
    try {
      ...
      throw new ParseException(
          "Exception in " + className + " constructor: " + ie.exception().referenceType().name());
    } catch (IncompatibleThreadStateException exc) {
      throw new ParseException("Thread not suspended");
    } catch (Exception e) {
      /*
  ...
  private static LValue nFields(LValue lval, StringTokenizer izer, ThreadReference thread)
      throws ParseException {
  ...
  static LValue makeName(VirtualMachine vm, ExpressionParser.GetFrame frameGetter, String name)
      throws ParseException {
        ...
        while (izer.hasMoreTokens()) {
          ...
          if (qualifiedType != null) {
            ...
            if (classes.size() > 0) {
              ...
              //                			System.err.println("When parsing expression " + name + ", I
              // cannot analyze the qualifedType " + qualifiedType);
              break;
            }
          } else {
            break;
  ...
  static LValue booleanOperation(VirtualMachine vm, Token token, LValue rightL, LValue leftL)
      throws ParseException {
