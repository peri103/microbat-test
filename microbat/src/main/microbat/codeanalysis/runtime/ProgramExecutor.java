...
/**
 * @author Yun Lin
 *     <p>This class origins from three classes written by LLT, i.e., BreakpointDebugger,
 *     JunitDebugger, and TestcaseExecutor.
 *     <p>This class is deprecated, please check the InstrumentationExecutor class.
 */
...
public class ProgramExecutor extends Executor {
  ...
  /**
   * Executing the program, each time of the execution, we catch a JVM event (e.g., step event,
   * class preparing event, method entry event, etc.). Generally, we collect the runtime program
   * states in some interesting step event, and record these steps and their corresponding program
   * states in a trace node. <br>
   * <br>
   * Note that the trace node can form a tree-structure in terms of method invocation relations and
   * loop relations. <br>
   * <br>
   * See the field <code>trace</code> in this class. <br>
   * The parameter of executionOrderList is used to make sure the trace is recorded as
   * executionOrderList shows. Different from when we record the executionOrderList, we will listen
   * to method entry/exist event, which can introduces some steps which executionOrderList does not
   * record. In such case, we just focus on the part of trace recorded by executionOrderList.
   *
   * @param runningStatements
   * @throws SavException
   */
  public void run(
      List<BreakPoint> runningStatements,
      List<BreakPoint> executionOrderList,
      IProgressMonitor monitor,
      int stepNum,
      boolean isTestcaseEvaluation)
   ...
   * @param classScope
   */
  private List<LocalVariableScope> parseLocalVariables(
      final List<String> classScope, AppJavaClassPath appPath) {
  ...
  class UsedVarValues {
    ...
    public UsedVarValues(
        List<VarValue> readVariables,
        List<VarValue> writtenVariables,
        VarValue returnedValue,
        UsedVariable usedVar) {
      super();
   ...
   */
  private VirtualMachine constructTrace(
      IProgressMonitor monitor,
      List<BreakPoint> executionOrderList,
     ...
     * called by normal library code.
     *
     * <p>To this end, when we detect the library code is called/accessed by some normal library
     * code, we set the flag <code>isIndirectAccess</code> to be k++. Then, we will not further
     * analyze the runtime variables even if the code is in our interested library.
     */
        ...
        if (event instanceof VMDeathEvent || event instanceof VMDisconnectEvent) {
          ...
          if (isInIncludedLibrary(currentLocation)) {
            if (trace.size() > 0 && (!isIndirectAccess || !Settings.applyLibraryOptimization)) {
              UsedVarValues uVars =
                  build3rdPartyLibraryDependency(thread, currentLocation, previousVars);
          ...
          if (trace.getLatestNode() != null) {
             ...
             *
             * <p>If the context changes, the value of some variables may not be retrieved. Thus,
             * the variable ID cannot be generated. Note that the ID of a variable need parsing its
             * heap ID which can only be accessed by runtime.
             */
            isContextChange = checkContext(trace.getLatestNode().getBreakPoint(), currentLocation);
            if (!isContextChange) {
              processWrittenVariable(
                  this.trace.getLatestNode(),
                  this.trace.getStepVariableTable(),
                  thread,
                  currentLocation);
            }
          }
          ...
          if (isMethodEntry) {
            if (prevNode != null && !isIndirectAccess) {
              parseWrittenParameterVariableForMethodInvocation(
                  thread, prevNode, trace.getLatestNode());
          ...
          if (caughtLocationForJustException != null) {
            popUpMethodCausedByException(
                methodNodeStack, methodSignatureStack, caughtLocationForJustException);
          ...
          if (monitor.isCanceled()
              || this.trace.size() >= Settings.stepLimit
   ...
   *
   * <p>the parameter <code>isPrecise</code> to specify whether the given trace step and some
   * invoker in method stack share same location or context.
   *
   * @param node
   ...
   * @return
   */
  private int checkContextualReturn(
      TraceNode node, Stack<TraceNode> methodNodeStack, boolean isPrecise) {
    int count = 0;
    boolean found = false;
  ...
  private void creatRWReturnVariableForReturnStatement(TraceNode prevNode, TraceNode node) {
    for (VarValue returnValue : prevNode.getReturnedVariables()) {
      ...
      List<StepVariableRelationEntry> entries =
          constructStepVariableEntry(trace.getStepVariableTable(), returnValue);
  ...
  private boolean isPointEndOfMethod(TraceNode node) {
    ...
    InstructionHandle lastHandle =
        visitor.getInstructionList().get(visitor.getInstructionList().size() - 1);
    ...
    for (CFGNode exitNode : cfg.getExitList()) {
      ...
      while (cfgNode.getParents().size() == 1) {
        ...
        if (parent.getChildren().size() == 1
            && !isParentInvokeAppMethod(parent, visitor.getMethod())) {
  ...
  private boolean isMethodExit(TraceNode prevNode, TraceNode thisNode, TraceNode peekNode) {
    ...
    /**
     * try to check whether the node after it is still a return node in the same method, if yes, the
     * method exit event for prev node should not happen.
     */
  ...
  private MethodDeclaration getMethodByAST(BreakPoint point) {
    ...
    if (method == null) {
      CompilationUnit thisCU =
          JavaUtil.findCompilationUnitInProject(
              point.getDeclaringCompilationUnitName(), this.appPath);
  ...
  /**
   * There is a conservative analysis for checking indirect access/invocation. More specifically, a
   * line such as the start of a class, can be corresponded to multiple byte code instructions or
   * associated with many byte code level method. It is dogmatic to infer which method can be
   * associated with this line in the static analysis. for now, once a line is the start of a class,
   * we set its method as unknown so that we can have a conservative analysis to figure out whether
   * two methods have invocation relationship. namely, an unknown method may invoke any methods and
   * be invoked by any methods.
   *
   * @param methodSignatureStack
   * @param methodNodeStack
   * @return
   */
  private boolean checkIndirectAccess(
      Stack<String> methodSignatureStack, Stack<TraceNode> methodNodeStack) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (node.getBreakPoint().isStartOfClass()
          || method.equals(ClassLocation.UNKNOWN_METHOD_SIGN)) {
        continue;
      }

      boolean isMethodInvokeRelationship = isMethodInvokeRelationship(node.getBreakPoint(), method);
  ...
  private boolean findInvokingMethod(
      String invokedMethodSig,
      SingleLineByteCodeVisitor visitor,
      List<InstructionHandle> peekList) {
    String invokedMethodName =
        invokedMethodSig.substring(
            invokedMethodSig.indexOf("#") + 1, invokedMethodSig.indexOf("("));
  ...
  private void appendReadVariableFromStepOver(TraceNode node) {
    ...
    if (previousStepOverNode != null
        && node.getLineNumber() == previousStepOverNode.getLineNumber()) {
      for (VarValue readVar : previousStepOverNode.getReadVariables()) {
        if (!node.containSynonymousReadVar(readVar)) {
          ...
          List<StepVariableRelationEntry> entries =
              constructStepVariableEntry(trace.getStepVariableTable(), readVar);
          for (StepVariableRelationEntry entry : entries) {
  ...
  private UsedVarValues build3rdPartyLibraryDependency(
      ThreadReference thread, Location currentLocation, UsedVariable previousVars) {
    ...
    for (VarValue writtenVar : uVars.writtenVariables) {
      ...
      for (VarValue v : list) {
        List<StepVariableRelationEntry> entries =
            constructStepVariableEntry(trace.getStepVariableTable(), v);
        for (StepVariableRelationEntry entry : entries) {
  ...
  class UsedVariable {
    ...
    public UsedVariable(
        ...
        List<Variable> writtenVariables,
        Variable returnedVar,
        String method) {
      super();
      this.readVariables = readVariables;
  ...
  private UsedVarValues parseUsedVariable(
      ThreadReference thread, Location currentLocation, UsedVariable previousVars) {
    ...
    if (previousVars != null && currentMethod.equals(previousVars.method)) {
      ...
      System.currentTimeMillis();
      List<VarValue> readArrayEleValues =
          parseValue(previousReadArrayElements, thread, Variable.READ);
    ...
    UsedVarValues uValues =
        new UsedVarValues(readVarValues, writtenVarValues, returnedValue, uVars);
    return uValues;
  }

  private List<VarValue> parseValue(
      List<Variable> vars, ThreadReference thread, String accessType) {
        ...
        if (value instanceof ObjectReference) {
          if (var instanceof ArrayElementVar) {
            ...
            if (sv instanceof ObjectReference) {
              ...
              String order =
                  trace.findDefiningNodeOrder(accessType, trace.getLatestNode(), varID, aliasVarID);
              varID = varID + ":" + order;
              ...
              String order =
                  trace.findDefiningNodeOrder(
                      accessType, trace.getLatestNode(), aliasVarID, var.getAliasVarID());
              aliasVarID = aliasVarID + ":" + order;
            ...
            String definingNodeOrder =
                this.trace.findDefiningNodeOrder(
                    accessType, trace.getLatestNode(), varID, var.getAliasVarID());
            ...
            if (value.type().toString().equals("java.lang.String")) {
              ...
              if (objRef instanceof ArrayReference) {
                ...
                varValue =
                    constructArrayVarValue(arrayValue, var, frame.thread(), null, accessType, 0);
              } else {
                varValue =
                    constructReferenceVarValue(objRef, var, frame.thread(), null, accessType, 0);
              }
              ...
              if (((FieldVar) var).isStatic()) {
                ...
                varID =
                    Variable.concanateFieldVarID(
                        String.valueOf(objRef.uniqueID()), var.getSimpleName());
              }
              String definingNodeOrder =
                  this.trace.findDefiningNodeOrder(accessType, trace.getLatestNode(), varID, null);
              varID = varID + ":" + definingNodeOrder;
              var.setVarID(varID);
            }
          }
  ...
  private void popUpMethodCausedByException(
  ...
  private void parseWrittenParameterVariableForMethodInvocation(
      ThreadReference thread, TraceNode invokerNode, TraceNode invokeeNode) {
    ...
    for (Param param : paramList) {
      ...
      LocalVar localVar =
          new LocalVar(param.getName(), param.getType(), compilationUnit, lineNumber);
      ...
      if (!(value instanceof ObjectReference) || value == null) {
        ...
        LocalVariableScope scope =
            parser.parseMethodScope(compilationUnit, lineNumber, localVar.getName(), appPath);
        String varID;
        if (scope != null) {
          varID =
              Variable.concanateLocalVarID(
                  compilationUnit, localVar.getName(), scope.getStartLine(), scope.getEndLine());
          String definingNodeOrder =
              this.trace.findDefiningNodeOrder(
                  Variable.WRITTEN, invokerNode, varID, localVar.getAliasVarID());
          varID = varID + ":" + definingNodeOrder;
        ...
        ObjectReference objRef = (ObjectReference) value;
        String varID = String.valueOf(objRef.uniqueID());
        //				String definingNodeOrder = this.trace.findDefiningNodeOrder(Variable.WRITTEN,
        // invokerNode,
        //						varID, null);
        //				varID = varID + ":" + definingNodeOrder;
        localVar.setVarID(varID);
        ...
        if (objRef instanceof ArrayReference) {
          ArrayReference arrayValue = (ArrayReference) objRef;
          varValue =
              constructArrayVarValue(
                  arrayValue,
                  localVar,
                  frame.thread(),
                  invokerNode.getBreakPoint(),
                  Variable.WRITTEN,
                  Settings.getVariableLayer());
        } else {
          varValue =
              constructReferenceVarValue(
                  objRef,
                  localVar,
                  frame.thread(),
                  invokerNode.getBreakPoint(),
  ...
  private void parseBreakpoints(
      VirtualMachine vm, ClassPrepareEvent classPrepEvent, Map<String, BreakPoint> locBrpMap) {
  ...
  private VarValue constructReferenceVarValue(
    ...
    String order =
        this.trace.findDefiningNodeOrder(
            accessType, trace.getLatestNode(), var.getVarID(), var.getAliasVarID());
    ...
    if (needParseFields) {
      ...
      for (Field field : fieldList) {
        ...
        if (!isIgnore) {
          FieldVar variable =
              new FieldVar(
                  false, field.name(), field.typeName(), field.declaringType().signature());
          extractor.appendVarVal(varValue, variable, map.get(field), retrieveLayer, thread, false);
        }
      }
    }
  ...
  private VarValue constructArrayVarValue(
      ArrayReference arrayValue,
      Variable var0,
      ThreadReference thread,
      BreakPoint point,
      String accessType,
  ...
  private VarValue generateVarValue(
      StackFrame frame, Variable var0, TraceNode node, String accessType, BreakPoint point) {
    VarValue varValue = null;
    /**
     * Note that the read/written variables in breakpoint should be different when set them into
     * different trace node.
     */
    Variable var = var0.clone();
      ...
      if (value instanceof ObjectReference) {
        ...
        if (value.type().toString().equals("java.lang.String")) {
          ...
          if (objRef instanceof ArrayReference) {
            ...
            varValue =
                constructReferenceVarValue(
                    objRef, var, frame.thread(), point, accessType, Settings.getVariableLayer());
          }
      ...
      /** its a primitive type */
          ...
          if (scope != null) {
            varID =
                Variable.concanateLocalVarID(
                    ...
                    scope.getEndLine());
            String definingNodeOrder =
                this.trace.findDefiningNodeOrder(accessType, node, varID, var.getAliasVarID());
            varID = varID + ":" + definingNodeOrder;
          }
        ...
        /** It's a field or array element. */
          ...
          if (var instanceof FieldVar) {
            ...
            String varID =
                Variable.concanateArrayElementVarID(
                    String.valueOf(objRef.uniqueID()), indexValueString);
            String definingNodeOrder =
                this.trace.findDefiningNodeOrder(accessType, node, varID, var.getAliasVarID());
            varID = varID + ":" + definingNodeOrder;
            var.setVarID(varID);
    ...
    synchronized (frame) {
      ...
      for (Variable readVar : readVariables) {
        ...
        if (varValue != null) {
          ...
          List<StepVariableRelationEntry> entries =
              constructStepVariableEntry(stepVariableTable, varValue);
          for (StepVariableRelationEntry entry : entries) {
            entry.addConsumer(node);
          }
        }
  ...
  private void processReturnVariable(TraceNode node, ThreadReference thread, Location location) {
    ...
    CompilationUnit cu =
        JavaUtil.findCompilationUnitInProject(node.getDeclaringCompilationUnitName(), appPath);
        ...
        if (exprString.contains("(") || exprString.contains("[")) {
          ...
          if (expr instanceof NullLiteral
            ...
            synchronized (frame) {
              ...
              VarValue valueV =
                  generateVarValue(frame, vVar, node, Variable.WRITTEN, node.getBreakPoint());
  ...
  private List<StepVariableRelationEntry> constructStepVariableEntry(
      Map<String, StepVariableRelationEntry> stepVariableTable, VarValue var) {
    List<StepVariableRelationEntry> list = new ArrayList<>();
    StepVariableRelationEntry entry =
        constructStepVariableEntry(stepVariableTable, var.getVarID(), var.getVariable());
    list.add(entry);

    if (var.getAliasVarID() != null) {
      StepVariableRelationEntry entry0 =
          constructStepVariableEntry(stepVariableTable, var.getAliasVarID(), var.getVariable());
      list.add(entry0);
    }
  ...
  private StepVariableRelationEntry constructStepVariableEntry(
      Map<String, StepVariableRelationEntry> stepVariableTable, String varID, Variable var) {
  ...
  private void processWrittenVariable(
      TraceNode node,
      ...
      for (Variable writtenVar : writtenVariables) {
        ...
        if (varValue != null) {
          ...
          for (VarValue v : list) {
            ...
            List<StepVariableRelationEntry> entries =
                constructStepVariableEntry(stepVariableTable, v);
            for (StepVariableRelationEntry entry : entries) {
  ...
  private ExpressionValue retriveExpression(
      final StackFrame frame0, String expression, BreakPoint point) {
    ...
    try {
      ...
      if (point == null) {
        ...
        cu =
            JavaUtil.findCompilationUnitInProject(point.getDeclaringCompilationUnitName(), appPath);
      }
  ...
  private void collectValueOfPreviousStep(
      BreakPoint lastSteppingInPoint, ThreadReference thread, Location loc) throws SavException {

    BreakPoint current =
        new BreakPoint(
            lastSteppingInPoint.getClassCanonicalName(),
            lastSteppingInPoint.getDeclaringCompilationUnitName(),
  ...
  private BreakPointValue extractValuesAtLocation(
      BreakPoint bkp, ThreadReference thread, Location loc) throws SavException {
