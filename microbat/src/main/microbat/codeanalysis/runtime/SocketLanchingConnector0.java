...
public class SocketLanchingConnector0 extends SocketLaunchingConnectorImpl {
  ...
  public SocketLanchingConnector0(VirtualMachineManagerImpl virtualMachineManager) {
  ...
  public String description() {
    return ConnectMessages
        .SocketLaunchingConnectorImpl_Launches_target_using_Sun_Java_VM_command_line_and_attaches_to_it_13;
  }
  ...
  public VirtualMachine launch(
      Map<String, ? extends Connector.Argument> connectionArgs,
    ...
    SocketListeningConnectorImpl listenConnector =
        new SocketListeningConnectorImpl(virtualMachineManager());
    ...
    // execString += " -Xrunjdwp:transport=dt_socket,address=" + address + ",server=n,suspend=" +
    // (fSuspend ? "y" : "n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    execString +=
        " -noverify -agentlib:jdwp=transport=dt_socket,address="
            + address
            + ",suspend="
            + (fSuspend ? "y" : "n"); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$

    // Add User specified options.
    if (fOptions != null) execString += " " + fOptions; // $NON-NLS-1$
    ...
    try {
      ...
      String message =
          NLS.bind(
              ConnectMessages
                  .SocketLaunchingConnectorImpl_VM_did_not_connect_within_given_time___0__ms_1,
              new String[] {
