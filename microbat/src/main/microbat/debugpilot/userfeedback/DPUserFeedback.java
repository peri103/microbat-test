...
public class DPUserFeedback {
  ...
  public DPUserFeedback(
      final DPUserFeedbackType type,
      final TraceNode node,
      final Collection<VarValue> wrongVars,
      Collection<VarValue> correctVars) {
  ...
  public void addCorrectVars(final Collection<VarValue> correctVars) {
    ...
    if (!this.wrongVars.isEmpty()) this.wrongVars.removeAll(correctVars);
  }
   ...
   * 2. If it is wrong variable, wrong variables set of otherFeedback should be the sub-set of this
   * wrong variable set
   *
  ...
  public boolean isSimilar(final DPUserFeedback otherFeedback) {
    ...
    if (this.type == DPUserFeedbackType.WRONG_VARIABLE
        && !this.wrongVars.containsAll(otherFeedback.wrongVars)) {
      return false;
    } else {
      return true;
    }
  ...
  //	public static DPUserFeedback parseFeedbacks(final TraceNode node, final UserFeedback...
  // feedbacks) {
  //		return DPUserFeedback.parseFeedbacks(node, Arrays.asList(feedbacks));
  //	}
  //
  //	public static DPUserFeedback parseFeedbacks(final TraceNode node, final
  // Collection<UserFeedback> userFeedbacks) {
  //		Objects.requireNonNull(userFeedbacks, Log.genMsg("DPUserFeedback", "Given userFeedbacks cannot
  // be null"));
  //
  //		if (userFeedbacks.isEmpty()) {
  //			throw new IllegalArgumentException(Log.genMsg("DPUserFeedback", "Cannot create DPUserFeedback
  // from no feedbacks"));
  //		}
  //
  //		// Check that there are no conflicting feedbacks
  //		if (userFeedbacks.stream().map(feedback ->
  // feedback.getFeedbackType()).distinct().limit(2).count() == 1) {
  //			throw new IllegalArgumentException(Log.genMsg("DPUserFeedback", "Cannot create DPUserFeedback
  // from conflicting feedbacks"));
  //		}
  //
  //		final String feedbackType = userFeedbacks.stream().map(feedback ->
  // feedback.getFeedbackType()).findFirst().orElse(null);
