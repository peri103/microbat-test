 ...
 *
 * <p>The main reason of using VarIDConverter is that, some of the character used in the variable ID
 * was used as the splitting delimiter when building the factor graph, so that we need to change
 * those character when we are using factor graph.
 *
 * <p>Also, we need to convert those character back after using the factor graph in order to
 * maintain the consistence of ID.
 *
 * @author David
 */
