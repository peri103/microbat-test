...
/**
 * Probability inference model is used to calculate the probability correctness based on different
 * cases
 *
...
public class InferenceModel {
  ...
  public double getProbability(final int targetIdx) {
    ...
    double sum = 0;
    //		System.out.println("targetIdx: " + targetIdx + " nums: " +
    // this.getCorrespondenceCaseNo(targetIdx));
    //		String message = "";
  ...
  /**
   * Check is the given list of variables are valid. Those constraints should have the same number
  ...
  private List<Integer> getCorrespondenceCaseNo(final int targetIdx) {
    ...
    for (int i = 0; i < maxLeft; i++) {
      ...
      for (int j : tempResult) result.add(j + temp);
    }
    return result;
  }
}
