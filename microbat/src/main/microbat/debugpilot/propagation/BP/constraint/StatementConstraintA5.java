...
/**
 * StatementConstraintA5 is the constraint for special case that the statement only have
 * read/written variables
 *
 * <p>If the given read/written variable and the control dominator is correct then the statement is
 * correct.
 *
 * @author David
 */
  ...
  public StatementConstraintA5(TraceNode node, VarValue var, double propProbability) {
    ...
    if (Constraint.countReadVars(node) != 0 && Constraint.countWrittenVars(node) != 0) {
      throw new WrongConstraintConditionException(
          "Node: "
              + node.getOrder()
              + " have both read and written variables. Statement Constraint A5 only handle the"
