 ...
 *
 * <p>It is a helper structure to calculate the probability for different cases
 *
 * @author Shiang Hwee
 */
public class BitRepresentation {
  ...
  public BitRepresentation(int size) {
    ...
    for (int i = 0; i < size; i++) representation[i] = false;
    this.size = size;
    this.cardinality = 0;
  }
  ...
  /**
   * Perform AND operation with the other bit representation. If the other bit representation is too
   * short for this representation, it will perform zero padding at the front for the other bit.
   *
  ...
  /**
   * Perform OR operation with the other bit representation If the other bit representation is too
   * short for this representation, it will perform zero padding at the front for the other bit.
   *
  ...
  public boolean set(int index) {
    ...
    if (index >= this.representation.length) {
      throw new IndexOutOfBoundsException(
          "index: " + index + " exceed representation size: " + this.representation.length);
    }

    if (representation[index]) return false;
  ...
  public void set(int start, int end) {
    ...
    if (start >= this.size || end > this.size) {
      throw new IndexOutOfBoundsException(
          "given range: (" + start + "," + end + ") exceed the representation size " + this.size);
    }

    for (; start < end; start++) this.set(start);
  }
  ...
  public boolean clear(int index) {
    ...
    if (index >= this.size) {
      throw new IndexOutOfBoundsException(
          "index: " + index + " exceed representation size: " + this.size);
    }
  ...
  public void clear(int start, int end) {
    ...
    for (; start < end; start++) this.clear(start);
  }
   ...
   */
  public static BitRepresentation padding(
      final BitRepresentation bitRepresentation, final int size) {
    if (size < bitRepresentation.size) {
      throw new IllegalArgumentException(
          "padding size: "
              + size
              + " should not be smaller than representation size: "
              + bitRepresentation.size);
    }
   ...
   * @param size Size of the bit. Given size must not be smaller than length of string. It will
   *     perform zero padding at front if the size is larger than length of string
  ...
  public static BitRepresentation parse(String s, int size) {
    ...
    if (padding < 0) throw new IllegalArgumentException("Size must be >= length of string");
    for (int i = 0; i < s.length(); i++) {
      char c = s.charAt(i);
      if (c == '1') br.set(i + padding);
    }
    return br;
  }
   ...
   * @param size Target size. Given size must not be smaller than length of integer bit. It will
   *     perform zero padding at front if the size is larger than length of bit
