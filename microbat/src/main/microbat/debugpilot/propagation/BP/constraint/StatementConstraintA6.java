...
/**
 * StatementConstraintA6 is the constraint for special case that the statement only have control
 * dominator
 *
 * <p>If the control dominator is correct, then the statement is correct.
 *
 * @author David
 */
  ...
  public StatementConstraintA6(TraceNode node, double propProbability) {
    ...
    if (Constraint.countReadVars(node) != 0 || Constraint.countWrittenVars(node) != 0) {
      throw new WrongConstraintConditionException(
          "Node: "
              + node.getOrder()
              + " have read or written variable. Cannot construct Statement Constraint A6");
    }
    ...
    if (node.getControlDominator() == null) {
      throw new WrongConstraintConditionException(
          "Node: "
              + node.getOrder()
              + " do not have control dominator. Cannot construct Statement Constraint A6");
    }
