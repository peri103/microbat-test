...
/**
 * StatementConstraintA4 is the constraint for special case that the statement only have
 * read/written variables
 *
 * <p>The constraint represent the case that, under the condition of control dominator is correct,
 * if the one of the read/written variable is wrong, then the statement is wrong
 *
 * @author David
 */
public class StatementConstraintA4 extends StatementConstraint {
  ...
  public StatementConstraintA4(TraceNode node, VarValue var, double propProbability) {
    ...
    if (Constraint.countReadVars(node) == 0 && Constraint.countWrittenVars(node) == 0) {
      throw new WrongConstraintConditionException(
          "Node: "
              + node.getOrder()
              + " do not have any read or written variables to construct the Statement Constraint"
              + " A4");
    }
    ...
    if (Constraint.countReadVars(node) != 0 && Constraint.countWrittenVars(node) != 0) {
      throw new WrongConstraintConditionException(
          "Node: "
              + node.getOrder()
              + " have both read and written variables. Statement Constraint A4 only handle the"
              + " case that the node have only read or written variable");
    }

    if (!node.isReadVariablesContains(var.getVarID())
        && !node.isWrittenVariablesContains(var.getVarID())) {
      throw new WrongConstraintConditionException(
          "Node: " + node.getOrder() + " does not contain the variable " + var.getVarID());
    }
  ...
  protected double calProbability(int caseNo) {
    ...
    /**
     * For constraint A4, the invalid case is that, under the condition that the control dominator
     * is correct, when there are wrong read/written variable, the statement is still correct.
     */
