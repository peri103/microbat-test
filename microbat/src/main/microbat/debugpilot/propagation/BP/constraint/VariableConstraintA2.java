 ...
 *
 * <p>If all the written variable, control dominator, and other read variable are correct, then the
 * target read variable is also correct
 *
 * @author David
 */
public class VariableConstraintA2 extends VariableConstraint {
  ...
  public VariableConstraintA2(TraceNode node, VarValue conclusionVar, double propProbability) {
    ...
    if (Constraint.countReadVars(node) == 0) {
      throw new WrongConstraintConditionException(
          "Node: "
              + node.getOrder()
              + " does not contain any read variable. Cannot construct Variable Constraint A2");
    }
    ...
    if (Constraint.countWrittenVars(node) == 0 && node.getControlDominator() == null) {
      throw new WrongConstraintConditionException(
          "Node: "
              + node.getOrder()
              + " do not have any written variable or control dominator to construct A2 Varaible"
              + " Constraint");
    }
  ...
  protected BitRepresentation genBitRepresentation(TraceNode node) {
    ...
    if (Constraint.countPreds(node) == 0) {
      throw new WrongConstraintConditionException(
          "The given node do not have any predicates. Cannot construct constraint.");
    }
  ...
  protected int _defineConclusionIdx(TraceNode node) {
    ...
    if (node.isReadVariablesContains(this.conclusionVar.getVarID())) {
      ...
      index =
          node.getReadVariables().size() + node.getWrittenVariables().indexOf(this.conclusionVar);
    }
    ...
    if (index == -1) {
      throw new WrongConstraintConditionException(
          "Trace Node: "
              + node.getOrder()
              + " do not contraint variable: "
              + this.conclusionVar.getVarID());
    }
