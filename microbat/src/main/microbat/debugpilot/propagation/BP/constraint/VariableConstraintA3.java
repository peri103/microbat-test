 ...
 *
 * <p>If all the read and written variables are correct, then the control dominator is correct as
 * well.
 *
 * @author David
 */
  ...
  public VariableConstraintA3(TraceNode node, double propProbability) {
    ...
    if (node.getControlDominator() == null) {
      throw new WrongConstraintConditionException(
          "Node: "
              + node.getOrder()
              + " do not have control dominator. Cannot construct Variable Constraint A3");
    }
    ...
    if (Constraint.countReadVars(node) == 0 && Constraint.countWrittenVars(node) == 0) {
      throw new WrongConstraintConditionException(
          "Node: "
              + node.getOrder()
              + " do not have any read or written variable to constraint Variable Constraint A3");
    }
