...
/**
 * Statement encoder is used to calculate the probability of correctness of statement instance
 * (trace node)
 *
...
public class StatementEncoder extends Encoder {
  ...
  protected List<Constraint> genVarToStatConstraints(TraceNode node) {
    ...
    //
    //			Constraint constraintA1 = new StatementConstraintA1(variableIncluded, conclusionIdx,
    // PropagationProbability.HIGH, writeStartIdx, statementOrder, controlDomID);
    //			constraintA1.setVarsID(node);
    //			constraints.add(constraintA1);
    //
    //		} else if ((readLen == 0 && writeLen != 0) ||(readLen != 0 && writeLen == 0)) { // 1st and
    // 2nd case
    //
    ...
    //			Constraint constraintA2 = new StatementConstraintA2(variableIncluded, conclusionIdx,
    // PropagationProbability.HIGH, writeStartIdx, statementOrder, controlDomID);
    //			constraintA2.setVarsID(node);
    //			constraints.add(constraintA2);
    //
    //			// Variable to statement constraint A3
    //			Constraint constraintA3 = new StatementConstraintA3(variableIncluded, conclusionIdx,
    // PropagationProbability.HIGH, writeStartIdx, statementOrder, controlDomID);
  ...
  protected List<Constraint> genPriorConstraints(TraceNode node) {
    ...
    //			Constraint constarint = new PriorConstraint(bitRep, predIdx,
    // controlDomValue.getProbability());
  ...
  protected List<Constraint> genSpecialCaseConstraints(TraceNode node) {
    ...
    //			}
    //
    //			Constraint constraint = new StatementConstraintA4(br, conclusionIdx,
    // PropagationProbability.HIGH, writeStartIdx, statementOrder, controlDomID);
  ...
  ////		Constraint constraint_forward = new StatementConstraint(variableIncluded, conclusionIdx,
  // Configs.HIGH, ConstraintType.PROG_STRUCTURE, readLen - 1);
  ////		Constraint constraint_backward = new StatementConstraint(variableIncluded, structureIdx,
  // Configs.HIGH, ConstraintType.PROG_STRUCTURE, readLen - 1);
  ...
  ////		Constraint priorConstraint = new StatementConstraint(varBit, structureIdx, prob,
  // ConstraintType.PRIOR, readLen - 1);
