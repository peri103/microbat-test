...
public class ProbInfer implements ProbabilityPropagator {
  ...
  public ProbInfer(final PropagatorSettings settings) {
    this(
        settings.getTrace(),
        settings.getSlicedTrace(),
        settings.getCorrectVars(),
        settings.getWrongVars(),
        settings.getFeedbacks());
  }

  public ProbInfer(
      Trace trace,
      List<TraceNode> slicedTrace,
      Set<VarValue> correctVars,
      Set<VarValue> wrongVars,
      Collection<DPUserFeedback> feedbackRecords) {
    ...
    Objects.requireNonNull(
        correctVars, Log.genMsg(getClass(), "Correct variables should not be null"));
  ...
  public void propagate() {
    ...
    VariableEncoderFG varEncoder =
        new VariableEncoderFG(
            this.trace, this.slicedTrace, this.correctVars, this.wrongVars, this.feedbackRecords);
    varEncoder.encode();
