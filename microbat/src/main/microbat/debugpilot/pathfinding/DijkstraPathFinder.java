...
public abstract class DijkstraPathFinder extends AbstractPathFinder {
  ...
  protected Graph<TraceNode, DPUserFeedback> constructGraph() {
    Graph<TraceNode, DPUserFeedback> directedGraph =
        new DirectedWeightedMultigraph<TraceNode, DPUserFeedback>(DPUserFeedback.class);
    ...
    if (directedGraph.vertexSet().size() != this.slicedTrace.size()) {
      throw new RuntimeException(
          Log.genMsg(
              this.getClass(),
              "Graph constructed does not match with sliced trace: "
                  + directedGraph.vertexSet().size()
                  + " : "
                  + this.slicedTrace.size()));
    }
  ...
  /**
   * Get the cost of choosing the given varValue as next step. Greedy method will choose the lowest
   * one
   *
