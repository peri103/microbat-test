...
public class Parameters {
  ...
  static {
    ...
    opts.addOption(
        new Option(AGLINMENT_FUNCTION, false, "align rows of excels that have the same key"));
    opts.addOption(new Option(COMPARISON_FUNCTION, false, "compare excels"));
    opts.addOption(
        OptionBuilder.withArgName(COMPARISON_KEYS)
            .isRequired(true)
            .hasArgs()
            .create(COMPARISON_KEYS));
    opts.addOption(
        OptionBuilder.withArgName(INPUT_EXCEL)
            .withDescription("excel files to compare")
            .hasArgs()
            .create(INPUT_EXCEL));
    opts.addOption(
        OptionBuilder.withArgName(COMPARISON_RULES)
            .withDescription("defined rules to compare excel records")
            .hasArgs()
            .create(COMPARISON_RULES));
    opts.addOption(
        OptionBuilder.withArgName(COMPARISION_COMBINATION_RULES)
            .withDescription("defined rules to compare excel records")
            .hasArgs()
            .create(COMPARISION_COMBINATION_RULES));
    opts.addOption(
        OptionBuilder.withArgName(COMPARISON_STATISTIC)
            .withDescription("defined rules to compare excel records")
            .hasArg(false)
