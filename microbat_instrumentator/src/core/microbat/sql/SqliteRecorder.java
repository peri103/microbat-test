...
public class SqliteRecorder extends SqliteServer implements TraceRecorder {
  ...
  /** */
  public String insertTrace(
      Trace trace, String runId, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
    PreparedStatement ps;
    String sql =
        "INSERT INTO Trace (trace_id, run_id,thread_id,thread_name,isMain,generated_time) "
            + "VALUES (?, ?, ?, ?,?,?)";
    ps = conn.prepareStatement(sql);
    closables.add(ps);
    int idx = 1;
  ...
  private void insertSteps(
      String traceId, List<TraceNode> exectionList, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
    String sql =
        "INSERT INTO Step (trace_id, step_order, control_dominator, step_in, step_over,"
            + " invocation_parent, loop_parent,location_id, read_vars, written_vars, time) VALUES"
            + " (?,?,?,?,?,?,?,?,?,?,?)";
  ...
  private void insertStepVariableRelation(
      Trace trace, String traceId, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
  ...
  private void insertLocation(
      String traceId, List<TraceNode> nodes, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
    String sql =
        "INSERT INTO location (location_id,trace_id, class_name, line_number, is_conditional,"
            + " is_return) VALUES (?,?, ?, ?, ?, ?)";
    PreparedStatement ps = conn.prepareStatement(sql);
    closables.add(ps);
    ...
    for (BreakPoint location : set) {
      ...
      ps.setString(
          idx++, location.getDeclaringCompilationUnitName() + "_" + location.getLineNumber());
      ps.setString(idx++, traceId);
