...
public class TraceRecorder01 extends DbService {
  ...
  public int insertTrace(
    ...
    String sql =
        "INSERT INTO Trace (project_name, project_version, "
  ...
  private void insertSteps(
      int traceId, List<TraceNode> exectionList, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
    String sql =
        "INSERT INTO Step (trace_id, step_order, control_dominator, step_in, step_over,"
            + " invocation_parent, loop_parent,location_id, read_vars, written_vars) VALUES"
            + " (?,?,?,?,?,?,?,?,?,?)";
  ...
  private void insertStepVariableRelation(
      Trace trace, int traceId, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
  ...
  private Map<TraceNode, Integer> insertLocation(
      int traceId, List<TraceNode> nodes, Connection conn, List<AutoCloseable> closables)
      throws SQLException {
    String sql =
        "INSERT INTO Location (trace_id, class_name, line_number, is_conditional, is_return) "
            + "VALUES (?, ?, ?, ?, ?)";
  ...
  private void insertControlScope(
      int traceId,
      Map<TraceNode, Integer> locationIdMap,
      Connection conn,
      List<AutoCloseable> closables)
      throws SQLException {
    String sql =
        "INSERT INTO ControlScope (trace_id, location_id, class_name, line_number, is_loop) VALUES"
            + " (?, ?, ?, ?, ?)";
    PreparedStatement ps = conn.prepareStatement(sql);
    closables.add(ps);
    int count = 0;
  ...
  private void insertLoopScope(
      ...
      throws SQLException {
    String sql =
        "INSERT INTO LoopScope (trace_id, location_id, class_name, start_line, end_line) VALUES (?,"
            + " ?, ?, ?, ?)";
    PreparedStatement ps = conn.prepareStatement(sql);
    closables.add(ps);
