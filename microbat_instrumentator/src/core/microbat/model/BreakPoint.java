...
public class BreakPoint extends ClassLocation {
  ...
  /**
   * The reason to differentiate control scope and loop scope is that (1) control scope include more
   * than a code block, e.g., a statement outside a block can be control dependent on a statement
   * inside a block. (2) in contrast, loop scope can only include the statements inside a code
   * block.
   */
  ...
  public BreakPoint(
      String className, String declaringCompilationUnitName, String methodSign, int lineNo) {
  ...
  public Object clone() {
    ...
    BreakPoint point =
        new BreakPoint(location.getClassCanonicalName(), declaringCompilationUnitName, lineNo);
  ...
  public String toString() {
    return "BreakPoint [classCanonicalName="
        + classCanonicalName
        + ", lineNo="
        + lineNo
        + ", methodName="
