...
public class BreakpointUtils {
  ...
  public static <T extends ClassLocation> Map<String, List<T>> initBrkpsMap(Collection<T> brkps) {
    Map<String, List<T>> brkpsMap = new HashMap<String, List<T>>();
    for (T brkp : brkps) {
      List<T> bps = CollectionUtils.getListInitIfEmpty(brkpsMap, brkp.getClassCanonicalName());
      bps.add(brkp);
    }
  ...
  public static <T extends ClassLocation> Map<String, List<Integer>> initLineNoMap(List<T> brkps) {
    Map<String, List<Integer>> map = new HashMap<String, List<Integer>>();
    for (T brkp : brkps) {
      List<Integer> lineNos = CollectionUtils.getListInitIfEmpty(map, brkp.getClassCanonicalName());
  ...
  public static <T extends ClassLocation> List<String> toLocationIds(Collection<T> bkps) {
  ...
  public static BreakPoint toBreakPoint(ClassLocation location) {
    BreakPoint breakpoint =
        new BreakPoint(
            location.getClassCanonicalName(), location.getMethodSign(), location.getLineNumber());
    return breakpoint;
  }
}
