...
public class LocalVariableScopes {
  ...
  public LocalVariableScope findScope(
      String variableName, int appearedLineNum, String fullQualifiedTypeName) {
    LocalVariableScope candScope = null;

    for (LocalVariableScope scope : variableScopes) {
      //			AbstractTypeDeclaration td = (AbstractTypeDeclaration)
      // scope.getCompilationUnit().types().get(0);
      //			String packageName =
      // scope.getCompilationUnit().getPackage().getName().getFullyQualifiedName();
      ...
      if (typeName.equals(fullQualifiedTypeName)
          && scope.getVariableName().equals(variableName)
          && appearedLineNum >= scope.getStartLine()
          && appearedLineNum <= scope.getEndLine()) {
