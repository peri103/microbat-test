...
public class PrecheckInstrumenter extends TraceInstrumenter {
  ...
  private void calculateTraceInstrumentation(
      JavaClass jc, String classFName, List<Method> methods) {
    ...
    for (Method method : methods) {
      ...
      try {
        changed =
            super.instrumentMethod(classGen, constPool, methodGen, method, true, false, false);
        ...
        if (e.getMessage() != null && e.getMessage().contains("offset too large")) {
          ...
          AgentLogger.info(
              String.format(
                  "Warning: %s [%s] - instrumentated bytecode exceeds limit",
                  classMethod, message));
        } else {
  ...
  protected boolean instrumentMethod(
    ...
    LocalVariableGen tracerVar =
        methodGen.addLocalVariable(
            MEASUREMENT_VAR_NAME,
            Type.getType(TraceMeasurement.class),
            insnList.getStart(),
            insnList.getEnd());
    ...
    for (LineNumberGen lineInfo : lineInsnInfos) {
      injectCodeTracerHitLine(
          ...
          constPool,
          tracerVar,
          lineInfo.getSourceLine(),
          lineInfo.getInstruction(),
          classNameVar,
          methodSigVar);
      for (InstructionHandle insn :
          LineInstructionInfo.getInstrInstructions(
              insnList, method.getLineNumberTable(), lineInfo.getSourceLine())) {
        injectCodeTracerHitLine(
            insnList,
            constPool,
            tracerVar,
        ...
        if (insn.getInstruction() instanceof InvokeInstruction) {
          InstructionList newInsns =
              getHitLineCode(
                  constPool, tracerVar, lineInfo.getSourceLine(), classNameVar, methodSigVar);
    ...
    injectCodeInitMeasurement(
        methodGen, constPool, classNameVar, methodSigVar, tracerVar, startLine, isMainMethod);
    instrumentedMethods.add(method);
    return true;
  }

  private void injectCodeTracerHitLine(
    ...
    InstructionList newInsns =
        getHitLineCode(constPool, tracerVar, sourceLine, classNameVar, methodSigVar);
    insertInsnHandler(insnList, newInsns, lineNumberInsn);
    newInsns.dispose();
  }

  private InstructionList getHitLineCode(
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      int sourceLine,
      LocalVariableGen classNameVar,
  ...
  private LocalVariableGen injectCodeInitMeasurement(
      MethodGen methodGen,
      ConstantPoolGen constPool,
      LocalVariableGen classNameVar,
      LocalVariableGen methodSigVar,
  ...
  private void appendTracerMethodInvoke(
      InstructionList newInsns,
      MeasurementMethods method,
      ConstantPoolGen constPool,
      boolean isStatic) {
    if (isStatic) {
      int index =
  ...
  private enum MeasurementMethods {
    GET_TRACER(
        "microbat/instrumentation/precheck/TraceMeasurement",
        "_getTracer",
