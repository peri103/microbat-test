...
public class CoverageAgent extends Agent {
  ...
  public void startup0(long vmStartupTime, long agentPreStartup) {
    ...
    CoverageSFlowGraph coverageFlowGraph =
        constructor.buildCoverageGraph(
            appClasspath,
            agentParams.getTargetMethod(),
            agentParams.getCdgLayer(),
  ...
  public void exitTest(String testResultMsg, String junitClass, String junitMethod, long threadId) {
    ...
    AgentLogger.debug(
        String.format(
            "Exit testcase %s, testIdx=%s, thread=%s",
            InstrumentationUtils.getMethodId(junitClass, junitMethod), testIdx, threadId));
    if (testIdx != null) {
