...
public class CoverageInstrumenter extends AbstractInstrumenter {
  ...
  protected boolean instrumentMethod(
    ...
    MethodInstructionsInfo instmInsns =
        MethodInstructionsInfo.getInstrumentationInstructions(
            insnList, method, classGen.getClassName());
    ...
    if (agentParams.collectConditionVariation()) {
      for (InstructionInfo cmpInsnInfo : instmInsns.getNotIntCmpInIfInsns()) {
        injectCodeTracerOnNotIntCmp(
            methodIdVar, tracerVar, cmpInsnInfo, constPool, insnList, methodGen);
      }
  ...
  private void injectCodeTracerOnNotIntCmp(
      LocalVariableGen methodIdVar,
      LocalVariableGen tracerVar,
      InstructionInfo instnInfo,
      ConstantPoolGen constPool,
    ...
    switch (opcode) {
      case Const.DCMPG:
      case Const.DCMPL:
        LocalVariableGen d1Var =
            methodGen.addLocalVariable(
                nextTempVarName(),
                new ArrayType(Type.DOUBLE, 1),
                instnInfo.getInsnHandler().getPrev(),
                instnInfo.getInsnHandler().getNext());
        LocalVariableGen d2Var =
            methodGen.addLocalVariable(
                nextTempVarName(),
                new ArrayType(Type.DOUBLE, 1),
                instnInfo.getInsnHandler().getPrev(),
        ...
        break;
      case Const.LCMP:
        LocalVariableGen l1Var =
            methodGen.addLocalVariable(
                nextTempVarName(),
                new ArrayType(Type.LONG, 1),
                instnInfo.getInsnHandler().getPrev(),
                instnInfo.getInsnHandler().getNext());
        LocalVariableGen l2Var =
            methodGen.addLocalVariable(
                nextTempVarName(),
                new ArrayType(Type.LONG, 1),
                instnInfo.getInsnHandler().getPrev(),
      ...
      case Const.FCMPL:
        ...
        appendTracerMethodInvoke(
            newInsns, CoverageTracerMethods.ON_FCMP, constPool); // value1, value2
        insertInsnHandler(insnList, newInsns, instnInfo.getInsnHandler());
        newInsns.dispose();
        break;
    }
  ...
  private void injectCodeTracerOnIf(
      LocalVariableGen methodIdVar,
      LocalVariableGen tracerVar,
      InstructionInfo instnInfo,
      ConstantPoolGen constPool,
  ...
  private void injectCodeTracerReachNode(
      LocalVariableGen methodIdVar,
      InstructionInfo instnInfo,
      LocalVariableGen tracerVar,
      ConstantPoolGen constPool,
  ...
  private LocalVariableGen injectCodeInitTracer(
      MethodGen methodGen,
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      LocalVariableGen methodIdVar,
    ...
    if (agentParams.getCoverageType() == CoverageCollectionType.BRANCH_COVERAGE) {
      ...
      appendTracerMethodInvoke(
          newInsns, CoverageTracerMethods.BRANCH_COVERAGE_GET_TRACER, constPool);
    } else {
      /* invoke _getTracer */
      newInsns.append(new ALOAD(methodIdVar.getIndex())); // methodId
      ...
      newInsns.append(
          new PUSH(
              constPool, TraceUtils.encodeArgNames(getArgumentNames(methodGen)))); // paramNamesCode
      newInsns.append(
          new PUSH(
              constPool,
  ...
  private void injectCodeTracerExitMethod(
      LocalVariableGen methodIdVar,
      LocalVariableGen tracerVar,
      ConstantPoolGen constPool,
      InstructionList insnList,
      InstructionHandle exitInsn,
      String methodId) {
    boolean isEntryPoint = this.entryPoint.equals(methodId);
    InstructionList newInsns = new InstructionList();
    newInsns.append(new ALOAD(tracerVar.getIndex()));
  ...
  protected void appendTracerMethodInvoke(
      InstructionList newInsns, CoverageTracerMethods method, ConstantPoolGen constPool) {
