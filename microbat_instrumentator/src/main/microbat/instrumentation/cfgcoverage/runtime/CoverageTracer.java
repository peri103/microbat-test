...
public class CoverageTracer implements ICoverageTracer, ITracer {
  ...
  public static volatile Map<Integer, List<MethodExecutionData>> methodExecsOnASingleTcMap =
      new HashMap<>();
    ...
    if (currentNode == null) {
      ...
      if (branch != null) {
        ...
        if (!currentNode.isAliasNode()) {
          AgentLogger.debug(
              String.format(
                  "cannot find branch %s:%d of node %d [testcase=%s]",
                  methodId, nodeIdx, currentNode.getCvgIdx(), testcase));
        }
        return;
      }
    }
  ...
  public void enterMethod(
    ...
    if (isEntryPoint && methodHierachyLevel == 0) {
      ...
      BreakPointValue methodInput =
          valueExtractor.extractInputValue(
              String.valueOf(testIdx),
              loc.getClassCanonicalName(),
              loc.getMethodSign(),
              paramTypeSignsCode,
              paramNamesCode,
              params,
    ...
    try {
      ...
      if (currentNode.getType() != Type.INVOKE_NODE) {
        throw new SavRtException(
            String.format(
                "Expect INVOKE_NODE node, get %s (%s)", currentNode.getType(), currentNode));
      }
  ...
  public static synchronized ICoverageTracer _getTracer(
      String methodId,
      boolean isEntryPoint,
      String paramNamesCode,
      String paramTypeSignsCode,
    ...
    try {
      ...
      tracer.enterMethod(
          methodId, paramTypeSignsCode, paramNamesCode, params, isEntryPoint, receiver);
      return tracer;
    } catch (Throwable t) {
      AgentLogger.error(t);
      return EmptyCoverageTracer.getInstance();
