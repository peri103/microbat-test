...
public class Branch implements Serializable {
  ...
  public static Branch getBrach(String branchId, CoverageSFlowGraph graph) {
    ...
    return of(
        graph.getNodeList().get(Integer.valueOf(fromTo[0])),
    ...
    if (branchType == null) {
      if (!fromNode.isConditionalNode()) {
        ...
        if (cfgNode.getInstructionHandle().getInstruction() instanceof BranchInstruction) {
          BranchInstruction branchInsn =
              (BranchInstruction) cfgNode.getInstructionHandle().getInstruction();
  ...
  public BranchCondition getBranchCondition() {
    ...
    if (cfgNode.getInstructionHandle().getInstruction() instanceof IfInstruction) {
      if (CollectionUtils.existIn(
          ...
          Const.LCMP)) {
        branchCond =
            BranchCondition.valueOf(
                cfgNode.getInstructionHandle().getPrev().getInstruction().getOpcode(),
                cfgNode.getInstructionHandle().getInstruction().getOpcode());
      } else {
        branchCond =
            BranchCondition.valueOf(cfgNode.getInstructionHandle().getInstruction().getOpcode());
      }
    } else {
  ...
  public boolean equals(Object obj) {
    ...
    if (fromNode.getCvgIdx() != other.fromNode.getCvgIdx()) return false;
    if (toNode.getCvgIdx() != other.toNode.getCvgIdx()) return false;
    return true;
  }
  ...
  public String toString() {
    return "Branch ["
        + fromNode.getCvgIdx()
        + "(line "
        + fromNode.getFirstCFGNode().getLineNo()
        + ")"
        + "  -->  "
        + toNode.getCvgIdx()
        + "(line "
        + toNode.getFirstCFGNode().getLineNo()
        + ")"
        + "]";
  }
