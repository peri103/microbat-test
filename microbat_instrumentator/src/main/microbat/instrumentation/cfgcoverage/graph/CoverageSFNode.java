...
public class CoverageSFNode implements IGraphNode<CoverageSFNode> {
  ...
  private List<Integer>
      content; // for a block node which contain all nodes in block from start to end.
  /* for conditional node */
  private volatile Map<CoverageSFNode, List<String>> coveredTestcasesOnBranches =
      new HashMap<CoverageSFNode, List<String>>();
  ...
  private String getBranchesString() {
    if (graph == null) {
      ...
      int i = 0;
      for (CoverageSFNode branch :
          sav.common.core.utils.CollectionUtils.nullToEmpty(branchTargets)) {
