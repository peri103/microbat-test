...
public class LocalVariableSupporter {
  ...
  public static void fillUpVariableTable(
      MethodGen methodGen, Method method, ConstantPoolGen constPool) {
    ...
    for (InstructionHandle insnHandler : methodGen.getInstructionList()) {
      ...
      if ((localVarTable == null)
          || (localVarTable.getLocalVariable(
                  insn.getIndex(), insnHandler.getPosition() + insn.getLength())
              == null)) {
    ...
    Collections.sort(
        localVariables,
        new Comparator<LocalVariableInfo>() {
