...
public abstract class AbstractInstrumenter {
  ...
  protected abstract boolean instrumentMethod(
      ClassGen classGen,
  ...
  protected LocalVariableGen createLocalVariable(
      String varName, MethodGen methodGen, ConstantPoolGen constPool) {
    InstructionList list = methodGen.getInstructionList();
    LocalVariableGen varGen =
        methodGen.addLocalVariable(varName, Type.STRING, list.getStart(), list.getEnd());
    return varGen;
  }

  protected InstructionHandle insertInsnHandler(
      InstructionList insnList, InstructionList newInsns, InstructionHandle insnHandler) {
  ...
  protected void updateTarget(
      InstructionHandle oldPos, InstructionHandle newStart, InstructionHandle newEnd) {
  ...
  protected LocalVariableGen createMethodParamTypesObjectArrayVar(
      MethodGen methodGen,
      ConstantPoolGen constPool,
      InstructionHandle startInsn,
      InstructionList newInsns,
      String varName) {
    /* init Object[] */
    LocalVariableGen argObjsVar =
        methodGen.addLocalVariable(
            varName, new ArrayType(Type.OBJECT, 1), startInsn, startInsn.getNext());
