...
/**
 * @author LLT This instrumenter is to instrument Thread class Thread.start() is to be instrumented
 *     to call Agent._onStartThread to determine whether the application is multi-thread or not.
 */
public class ThreadInstrumenter extends AbstractInstrumenter {
  ...
  private boolean instrumentStartMethod(
      ClassGen classGen, ConstantPoolGen constPool, MethodGen methodGen, Method startMethod) {
    InstructionList newInsns = new InstructionList();
    int index =
        constPool.addInterfaceMethodref(
            Agent.class.getName().replace(".", "/"), "_onStartThread", "()V");
