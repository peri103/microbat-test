...
public class TraceInstrumenter extends AbstractInstrumenter {
      ...
      try {
        ...
        GeneratedMethods generatedMethods =
            runMethodInstrumentation(
                classGen, constPool, methodGen, method, isAppClass, isMainMethod, isEntry);
        if (generatedMethods != null) {
          if (doesBytecodeExceedLimit(generatedMethods)) {
            AgentLogger.info(
                String.format(
                    "Warning: %s exceeds bytecode limit!",
  ...
  private GeneratedMethods runMethodInstrumentation(
      ClassGen classGen,
    ...
    String methodFullName =
        MicrobatUtils.getMicrobatMethodFullName(classGen.getClassName(), method);
    boolean changed =
        instrumentMethod(classGen, constPool, methodGen, method, isAppClass, isMainMethod, isEntry);
   ...
   */
  protected boolean instrumentMethod(
      ClassGen classGen,
      ConstantPoolGen constPool,
      MethodGen methodGen,
      Method method,
      ...
      boolean isMainMethod,
      boolean isEntry) {
    if (!userFilters.isInstrumentable(
        classGen.getClassName(), method, methodGen.getLineNumbers())) {
      return false;
    }
    tempVarIdx = 0;
    ...
    List<LineInstructionInfo> lineInsnInfos =
        LineInstructionInfo.buildLineInstructionInfos(
            classGen, constPool, methodGen, method, isAppClass, insnList);
    ...
    for (LineInstructionInfo lineInfo : lineInsnInfos) {
      ...
      //			BreakPoint bp = new BreakPoint(classGen.getClassName(), classGen.getClassName() + "#" +
      // method.getName(), lineInfo.getLine());
      //			instructionTable.put(bp.hashCode(), lineInfo.getSerializable());

      injectCodeTracerHitLine(
          insnList,
          constPool,
          tracerVar,
          lineInfo.getLine(),
      ...
      for (RWInstructionInfo rwInsnInfo : rwInsns) {
        ...
        if (rwInsnInfo instanceof FieldInstructionInfo) {
          newInsns =
              getInjectCodeTracerRWriteField(
                  constPool,
                  tracerVar,
                  (FieldInstructionInfo) rwInsnInfo,
                  classNameVar,
                  methodSigVar);
        } else if (rwInsnInfo instanceof ArrayInstructionInfo) {
          newInsns =
              getInjectCodeTracerRWriteArray(
                  methodGen,
                  constPool,
                  tracerVar,
                  (ArrayInstructionInfo) rwInsnInfo,
                  classNameVar,
                  methodSigVar);
        } else if (rwInsnInfo instanceof LocalVarInstructionInfo) {
          if (rwInsnInfo.getInstruction() instanceof IINC) {
            newInsns =
                getInjectCodeTracerIINC(
                    constPool,
                    tracerVar,
                    (LocalVarInstructionInfo) rwInsnInfo,
                    classNameVar,
                    methodSigVar,
                    methodGen.isStatic());
          } else {
            newInsns =
                getInjectCodeTracerRWLocalVar(
                    constPool,
                    tracerVar,
                    (LocalVarInstructionInfo) rwInsnInfo,
                    classNameVar,
      ...
      for (InstructionHandle insn : lineInfo.getReturnInsns()) {
        injectCodeTracerReturn(
            insnList, constPool, tracerVar, insn, line, classNameVar, methodSigVar);
      }

      /** instrument exit instructions */
      for (InstructionHandle exitInsHandle : lineInfo.getExitInsns()) {
        injectCodeTracerExit(
            exitInsHandle,
            insnList,
            constPool,
            tracerVar,
  ...
  private void injectCodeTracerExit(
      InstructionHandle exitInsHandle,
    ...
    if (isMainMethod || isEntry) {
      int index =
          constPool.addInterfaceMethodref(
              Agent.class.getName().replace(".", "/"), "_exitProgram", "(Ljava/lang/String;)V");
      newInsns.append(new ALOAD(classNameVar.getIndex()));
      newInsns.append(new ALOAD(methodSigVar.getIndex()));
  ...
  private void injectCodeTracerReturn(
      InstructionList insnList,
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
  ...
  private void injectCodeTracerInvokeMethod(
      MethodGen methodGen,
      InstructionList insnList,
      ConstantPoolGen constPool,
    ...
    for (int i = 0; i < argTypes.length; i++) {
      ...
      if (argType instanceof BasicType) {
        newInsns.append(
            instructionFactory.createCheckCast(
  ...
  private void injectCodeAfterInvoke(
      InstructionList insnList,
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      InstructionHandle insnHandler,
    ...
    if (isInvokeStatic) {
      ...
      if (Type.VOID.equals(returnType)) {
        ...
        newInsns.append(
            new ALOAD(tracerVar.getIndex())); // [returnValue], objectRef, returnValue, $trace
        ...
        basicTypeSupporter.appendObjectConvertInstruction(
            returnType, newInsns, constPool); // returnValue*, objectRef, returnValue
        newInsns.append(
            new ALOAD(tracerVar.getIndex())); // [returnValue*], objectRef, returnValue, $trace
        newInsns.append(new DUP_X2()); // [returnValue*], $trace, objectRef, returnValue, $trace
        newInsns.append(new POP()); // [returnValue*], $trace, objectRef, returnValue
  ...
  private InstructionList getInjectCodeTracerRWriteField(
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
  ...
  private InstructionList getInjectCodePutField(
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      FieldInstructionInfo info,
      LocalVariableGen classNameVar,
   ...
   * @param classNameVar
   */
  private InstructionList getInjectCodePutStatic(
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      FieldInstructionInfo info,
      LocalVariableGen classNameVar,
  ...
  protected void appendTracerMethodInvoke(
      InstructionList newInsns, TracerMethods method, ConstantPoolGen constPool) {
    if (method.isInterfaceMethod()) {
      int index =
          constPool.addInterfaceMethodref(
              method.getDeclareClass(), method.getMethodName(), method.getMethodSign());
  ...
  private InstructionList getInjectCodeGetField(
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      FieldInstructionInfo info,
      LocalVariableGen classNameVar,
  ...
  private InstructionList getInjectCodeGetStatic(
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      FieldInstructionInfo info,
      LocalVariableGen classNameVar,
  ...
  private InstructionList getInjectCodeTracerRWLocalVar(
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
    ...
    /* invoke tracer */
    if (insnInfo.isStoreInstruction()
        && insnInfo.getInstructionHandler().getPrev().getInstruction() instanceof ACONST_NULL) {
  ...
  private InstructionList getInjectCodeTracerIINC(
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      LocalVarInstructionInfo insnInfo,
      LocalVariableGen classNameVar,
  ...
  private InstructionList getInjectCodeTracerRWriteArray(
      MethodGen methodGen,
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      ArrayInstructionInfo info,
      LocalVariableGen classNameVar,
      LocalVariableGen methodSigVar) {
      ...
      tempVarStartPos =
          newInsns.append(
              InstructionFactory.createStore(
                  info.getElementType(), arrEleTempVar.getIndex())); // arrRef,
      // idx
    ...
    if (info.getElementType() instanceof BasicType) {
      newInsns.append(
          new INVOKESTATIC(
              basicTypeSupporter.getValueOfMethodIdx(
                  (BasicType) info.getElementType(), constPool)));
    }
  ...
  private LocalVariableGen addTempVar(
      MethodGen methodGen, Type type, InstructionHandle insnHandler) {
    return methodGen.addLocalVariable(nextTempVarName(), type, insnHandler, insnHandler.getNext());
  }

  protected void injectCodeTracerHitLine(
      InstructionList insnList,
      ConstantPoolGen constPool,
      LocalVariableGen tracerVar,
      int line,
      InstructionHandle lineNumberInsn,
      LocalVariableGen classNameVar,
      LocalVariableGen methodSigVar,
  ...
  protected LocalVariableGen injectCodeInitTracer(
      MethodGen methodGen,
      ConstantPoolGen constPool,
      int methodStartLine,
      int methodEndLine,
    ...
    LocalVariableGen argObjsVar =
        createMethodParamTypesObjectArrayVar(
            methodGen, constPool, startInsn, newInsns, nextTempVarName());
    newInsns.append(new ALOAD(argObjsVar.getIndex()));
