 ...
 * @author LLT
 *     <p>Agent
 *     <p>* Record the testcases running process: _startTest: _finishTest: _exitProgram:
 */
public abstract class Agent {
  ...
  public abstract void retransformBootstrapClasses(
      Instrumentation instrumentation, Class<?>[] retransformableClasses) throws Exception;

  public abstract void exitTest(
      String testResultMsg, String junitClass, String junitMethod, long threadId);
  ...
  public static void _exitProgram(String programMsg) {
    if (Thread.currentThread().getName().equals("main")) {
      ...
      Runtime.getRuntime()
          .exit(1); // force program to exit to avoid getting stuck by background running threads.
    } else {
  ...
  private static Class<?>[] getRetransformableClasses(Instrumentation inst) {
    ...
    List<String> includedLibraryClasses =
        GlobalFilterChecker.getInstance().getIncludedLibraryClasses();
