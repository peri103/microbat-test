...
public class Premain {
  ...
  private static void installBootstrap(Instrumentation inst) throws Exception {
    ...
    if (bootJarPaths.isEmpty()) {
      bootJarPaths =
          getJarFiles(
              INSTRUMENTATION_STANTDALONE_JAR,
              "bcel-6.0.jar",
              "javassist.jar",
              SAV_JAR,
  ...
  private static List<JarFile> getJarFiles(String... jarNames) throws Exception {
    ...
    for (String jarName : jarNames) {
      ...
      if ((!isUptodate
              && CollectionUtils.existIn(file.getName(), INSTRUMENTATION_STANTDALONE_JAR, SAV_JAR))
          || !file.exists()) {
        try {
  ...
  public static boolean extractJar(String jarResourcePath, String filePath) throws IOException {
    final InputStream inputJarStream =
        Premain.class.getClassLoader().getResourceAsStream(jarResourcePath);
