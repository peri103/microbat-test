...
public class SimpleProgramsCorr {
  ...
  /**
   * Check whether a number is a palindrome 121 is a palindrome but 123 is not convert to list of
   * digits and check
   *
   * @param number
   * @return
  ...
  /**
   * Check whether a number is a palindrome 121 is a palindrome but 123 is not build the reverse
   * number and check they are the same
   *
   * @param number
   * @return
   */
  ...
  /**
   * array length n of numbers from 0-n-1 return number which appears more than once
   *
   * @param numbers
   * @return
   */
  ...
  /**
   * the matrix is sorted as 1 3 5 7 9 11 13 15 17 left to right, and the last number in each row is
   * not greater than the first number of the next row
   *
   * @param matrix
   * @param number
   * @return
  ...
  private boolean match(char[] s, int startS, char[] pattern, int startP) {
    ...
    if (startS >= s.length && startP < pattern.length) {
      return (startP + 1 < pattern.length
          && pattern[startP + 1] == START
          && match(s, startS, pattern, startP + 2));
    }
    ...
    if (startP + 1 >= pattern.length) {
      return (pattern[startP] == DOT || s[startS] == pattern[startP])
          && match(s, startS + 1, pattern, startP + 1);
    } else if (pattern[startP + 1] == START) {
      if (pattern[startP] == DOT || s[startS] == pattern[startP]) {
        return match(s, startS, pattern, startP + 2)
            || // no char in s matches ?*
            match(s, startS + 1, pattern, startP)
            || // one char in s matches  but still have ?*
            match(
                s,
                startS + 1,
                pattern,
                startP + 2); // one char in s matches and finish .?* where ? can be . or any char
      } else {
        return match(s, startS, pattern, startP + 2);
      }
    } else {
