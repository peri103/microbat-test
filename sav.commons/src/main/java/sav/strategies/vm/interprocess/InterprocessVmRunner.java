...
/**
 * @author LLT This VmRunner support interprocess communication. example: ServerInputWriter
 *     inputWriter = new ServerInputWriter(); ServerOutputReader outputReader = new
 *     ServerOutputReader(); InterprocessVmRunner vmRunner = new InterprocessVmRunner(inputWriter,
 *     outputReader); try { vmRunner.start(); inputWriter.sendData(...)); output =
 *     outputReader.readOutput(); } finally { vmRunner.stop(); inputWriter.close();
 *     outputReader.close(); }
 *     <p>Extend ServerInputWriter and ServerOutputReader for your particular purpose. Note: in case
 *     of Python, make sure you flush the stdout in order for main process to read! using
 *     stdout.flush()
 */
