...
/**
 * The code of this class is copied from eclipse JDT project, in order to make the whole Ziyuan
 * project works in eclipse plugin.
 *
 * @author "linyun"
 */
public class BootstrapPlugin {
  ...
  public static synchronized com.sun.jdi.VirtualMachineManager virtualMachineManager() {
    if (fVirtualMachineManager != null) return fVirtualMachineManager;
    ...
    try {
      ...
      if (extensionRegistry != null) { // is null if the platform was not
        ...
        className =
            extensionRegistry
                .getExtensionPoint(JDIDebugPlugin.getUniqueIdentifier(), "jdiclient")
                .getLabel(); //$NON-NLS-1$
      }
      Class<?> clazz = null;
      if (className != null) {
