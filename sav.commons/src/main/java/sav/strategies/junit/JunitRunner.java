...
public class JunitRunner {
  ...
  private static Request toRequest(Pair<String, String> pair) throws ClassNotFoundException {
  ...
  public static JunitResult runTestcases(JunitRunnerParameters params)
    ...
    List<Pair<String, String>> classMethods = JunitUtils.toPair(params.getClassMethods());
  ...
  private static String getLastFailureTrace(List<Failure> failures) {
    ...
    StackTraceElement firstStacktraceEle =
        CollectionUtils.getFirstElement(lastFailures.getException().getStackTrace());
  ...
  private static void extractBrkpsFromTrace(
      List<Failure> falures, JunitRunnerParameters params, Set<BreakPoint> bkps) {
    Set<String> acceptedClasses = new HashSet<String>();
    List<String> testingClassNames = CollectionUtils.initIfEmpty(params.getTestingClassNames());
    List<String> testingPkgs = CollectionUtils.initIfEmpty(params.getTestingPkgs());
    ...
    for (Failure failure : falures) {
      for (StackTraceElement trace : failure.getException().getStackTrace()) {
        String className = trace.getClassName();
        int lineNumber = trace.getLineNumber();
        if (className == null) {
          continue;
        }
        if (acceptedClasses.contains(className) || testingClassNames.contains(className)) {
          bkps.add(new BreakPoint(className, trace.getMethodName(), lineNumber));
          continue;
        }
  ...
  public static JunitResult runTestcases(
      AppJavaClassPath appClasspath, JunitRunnerParameters params)
  ...
  public static JunitResult runTestcases(VMConfiguration config, JunitRunnerParameters params)
      throws ClassNotFoundException, IOException, SavException {
    ...
    List<String> args =
        new JunitRunnerProgramArgBuilder()
  ...
  public static class JunitRunnerProgramArgBuilder extends ProgramArgumentBuilder {
    ...
    public JunitRunnerProgramArgBuilder testcaseTimeout(long timeout, TimeUnit unit) {
