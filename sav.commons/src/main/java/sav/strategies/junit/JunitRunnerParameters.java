...
public class JunitRunnerParameters {
  ...
  private static Option classMethods() {
    return OptionBuilder.withArgName(CLASS_METHODS)
        .withDescription("methods with full name includes className and method name")
        .hasArgs()
        .isRequired()
  ...
  private static Option timeout() {
    return OptionBuilder.withArgName(TESTCASE_TIME_OUT)
  ...
  private static Option testingPkgs() {
    return OptionBuilder.withArgName(TESTING_PACKAGE_NAMES)
        .withDescription("Testing packages contain classes for extracting failure traces")
        .hasArgs()
        .isRequired(false)
  ...
  private static Option destfile() {
    return OptionBuilder.withArgName(DEST_FILE)
  ...
  private static Option testingClassNames() {
    return OptionBuilder.withArgName(TESTING_CLASS_NAMES)
        .withDescription("Testing class names for extracting failure traces")
        .hasArgs()
        .isRequired(false)
  ...
  private static Option singleTestResultDetail() {
    return OptionBuilder.withArgName(SINGLE_TEST_RESULT_DETAIL)
  ...
  /**
   * all test methods in the given junit classes will be detected automatically and add to the
   * classMethods for the test.
   */
  public void setJunitClasses(List<String> junitClassNames) {
    try {
      ...
      throw new SavRtException("cannot extract test methods from junit classes: ", e.getMessage());
    }
  }
  ...
  public String toString() {
    return "JunitRunnerParameters [classMethods="
        + classMethods
        + ", testingClassNames="
        + testingClassNames
        + ", testingPkgs="
