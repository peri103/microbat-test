...
public abstract class ExecValue implements GraphNode {
  ...
  private void findValidatePathsToRoot(
      ExecValue node, ArrayList<ExecValue> path, ArrayList<ArrayList<ExecValue>> paths) {
    path.add(node);
   ...
   */
  public void retrieveValue(Map<String, double[]> allLongsVals, int i, int size) {
  ...
  /** TODO: to improve, varId of a child is always started with its parent's varId */
