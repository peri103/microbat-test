...
public class AtomUtils {
  ...
  /**
   * A = x (>, >=) a && B = x (>, >=) b, a > b A = x (<, <=) a && B = x (<, <=) b, a < b => A bounds
   * B
   */
  private static boolean isBound(Operator a, Operator b, double aConst, double bConst) {
    return (a.isGT() && b.isGT() && aConst > bConst) || (a.isLT() && b.isLT() && aConst < bConst);
  }

  private static boolean isNegation(Operator a, Operator b, double aConst, double bConst) {
    return (aConst == bConst) && (a.negative() == b);
  }

  private static boolean isHalfNegation(Operator a, Operator b, double aConst, double bConst) {
