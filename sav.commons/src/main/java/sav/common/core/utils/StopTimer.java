...
/**
 * A countdown timer which starts to work with the first entry and prints the results ascending with
 * the overall time.
 */
public class StopTimer {
  ...
  public synchronized LinkedHashMap<String, Long> getTimeResults() {
    ...
    while (iterator.hasNext()) {
      ...
      results.put(
          firstEntry.getValue(), getExecutionTime(firstEntry, lastEntry.getKey().longValue()));
  ...
  private String toDisplayString(Entry<String, Long> timeResult) {
    return new StringBuilder(module)
        .append(" - ")
        .append(timeResult.getKey())
        .append(": ")
        .append(getTimeString(timeResult.getValue()))
        .toString();
  }
