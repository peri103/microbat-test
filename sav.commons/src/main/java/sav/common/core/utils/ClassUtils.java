...
public class ClassUtils {
  ...
  /**
   * very weak method. only handle very simple case of className. TODO LLT: handle for the case of
   * inner class as well.
   */
  ...
  public static String getJFilePath(String className) {
    return className.replace(Constants.DOT, Constants.FILE_SEPARATOR) + Constants.JAVA_EXT_WITH_DOT;
  }
  ...
  public static List<File> getCompiledClassFiles(String targetPath, String className) {
    ...
    String classSimpleName = className.substring(lastDotIdx + 1).split("$")[0];
    ...
    Collection<File> files =
        FileUtils.listFiles(
            new File(classFolder),
            new WildcardFileFilter(
                new String[] {
                  getClassFileName(classSimpleName), getClassFileName(classSimpleName + "$*")
                }),
            null);
  ...
  public static Pair<String, String> splitClassMethod(String name) {
    ...
    if (idx > 0) {
      return Pair.of(name.substring(0, idx), name.substring(idx + 1));
    }
    throw new IllegalArgumentException(
        "Cannot extract method from string, expect [classname].[method], get " + name);
  }
  ...
  public static List<Method> loockupMethodByNameOrSign(Class<?> clazz, String methodNameOrSign) {
    ...
    if (matchingMethods.isEmpty()) {
      ...
      throw new IllegalArgumentException(
          String.format("cannot find method %s in class %s", methodNameOrSign, clazz.getName()));
    }
